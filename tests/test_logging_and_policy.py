...
def test_audit_event_writes_jsonl(tmp_path, monkeypatch):
    ...
    try:
        ...
        rec = json.loads(line)
        assert (
            rec["step"] == "unit_test" and rec["status"] == "OK" and rec["foo"] == "bar"
        )
    finally:
        os.chdir(cwd)
...
def test_run_streamed_logs_and_tail(tmp_path):
    ...
    rc = run_streamed(
        ["python", "-c", "print('hello');"], log_path=str(log_path), check=True
    )
...
def test_policy_required_vs_optional(tmp_path, monkeypatch):
    ...
    try:
        ...
        raised = False
        try:
            rp.pipeline_cfg = rp.load_pipeline_config(
                "conf/pipeline.yaml"
            )  # ensure it picks our temp config
            rp.run_step(
                "must_pass", bad, notes="testing required", pipeline_cfg=rp.pipeline_cfg
            )
        except Exception:
            raised = True
        ...
        st = rp.run_step(
            "can_warn", bad, notes="testing warn", pipeline_cfg=rp.pipeline_cfg
        )
        assert st == "PARTIAL"

        # skip: should return SKIP (no raise)
        st2 = rp.run_step(
            "can_skip", bad, notes="testing skip", pipeline_cfg=rp.pipeline_cfg
        )
        assert st2 == "SKIP"
