    ...
    try:
        ...
        if os.path.exists(test_video):
            try:
                ...
                print(f"✅ Audio stream probed successfully: {stream_info.get('codec_name', 'unknown')}")
    ...
    try:
        ...
        if os.path.exists(test_script):
            ...
            try:
                ...
                if result["success"]:
                    print(f"✅ SRT generated successfully: {result['generation_method']}")
...
def test_legibility_validation():
    ...
    try:
        ...
        for text_color, bg_color, test_name in test_cases:
            ...
            print(f"{status} {test_name}: contrast {result.get('contrast_ratio', 'N/A')}")
        
        # Test background injection
        result = inject_safe_background("#000000", "test_element")
        if result["success"]:
            print(f"✅ Background injection successful: {result['injected_background']}")
        else:
...
def test_configuration():
    ...
    try:
        ...
        if os.path.exists(render_path):
            ...
            acceptance = config.get("acceptance", {})
            required_settings = ["tolerance_pct", "audio_validation_required", "caption_validation_required"]
