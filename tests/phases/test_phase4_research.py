...
def test_intent_templates():
    ...
    try:
        ...
        print(
            f"✅ {intent} intent: CTA={metadata['cta_policy']}, Evidence={metadata['evidence_load']}"
        )

        # Test citation requirements
        citation_reqs = get_citation_requirements(intent)
        print(
            f"✅ Citation requirements: {citation_reqs['beats_needing_citations']}/{citation_reqs['total_beats']} beats need citations"
        )
...
def test_fact_guard():
    ...
    try:
        # Test claim analysis
        test_line = (
            "The topic was researched thoroughly with multiple credible sources."
        )
        ...
        config = load_config()
        claim_policies = (
            config.get("research", {}).get("fact_guard", {}).get("claim_policies", {})
        )
        ...
        for claim in claims:
            action = determine_claim_action(
                claim, {"rewrite_to_cautious": True}, claim_policies
            )
...
def main():
    ...
    if passed == total:
        print(
            "🎉 All tests passed! Phase 4 research implementation is working correctly."
        )
        return 0
    else:
