...
from bin.timing_utils import compute_scene_durations, validate_duration_tolerance
...
def test_scene_duration_computation():
    ...
    brief = load_brief()
    print(
        f"Brief target: {brief.get('video', {}).get('target_length_min', 'unknown')} minutes"
    )
    ...
    if beats_path.exists():
        ...
        durations, strategy, rationale = compute_scene_durations(
            beats, brief, timing_config, "test_slug"
        )
        ...
        is_valid, deviation, message = validate_duration_tolerance(
            total_ms, target_ms, 5.0
        )
        print(
            f"Tolerance validation: {is_valid} (deviation: {deviation:.1f}%) - {message}"
        )
...
def test_acceptance_duration_validation():
    ...
    print("Duration validation result:")
