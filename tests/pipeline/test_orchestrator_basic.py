...
async def test_orchestrator_basic():
    ...
    if db.update_job_status(job.id, JobStatus.RUNNING, Stage.RESEARCH):
        ...
        if (
            ...
            and updated_job.stage == Stage.RESEARCH
        ):
    ...
    test_event = Event(
        event_type="test_event", message="Test event message", metadata={"test": True}
    )
    ...
    if db.add_event(job.id, test_event):
        ...
        if events:
            ...
            print(
                f"   Latest event: {latest_event.event_type} - {latest_event.message}"
            )
        else:
    ...
    if db.create_or_update_gate(
        job.id, Stage.SCRIPT, True, "test-operator", "Test approval"
    ):
        ...
        if not gate_found:
            ...
            for gate in updated_job.gates:
                print(
                    f"     - {gate.stage.value}: approved={gate.approved}, by={gate.by}"
                )
            return
    else:
    ...
    if orchestrator:
        ...
        if job_status:
            print(
                f"   ✓ Retrieved job status via orchestrator: {job_status.status.value}"
            )
        else:
