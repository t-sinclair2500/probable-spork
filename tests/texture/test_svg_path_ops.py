    ...
    try:
        ...
        if os.path.exists(test_svg_path):
            ...
            if path:
                ...
                scaled_path = processor.transform_path(
                    path, "scale", scale_x=1.5, scale_y=1.5
                )
            ...
            if os.path.exists(base_asset):
                ...
                for motif_type in motif_types:
                    try:
                        ...
                        if variants:
                            print(
                                f"  ✓ Generated {len(variants)} {motif_type} variants"
                            )
        ...
        if generated_dir.exists():
            ...
            for svg_file in svg_files[:5]:  # Check first 5 files
                ...
                if 100 < file_size < 10000:  # Reasonable range for SVG
                    print(
                        f"  ✓ {svg_file.name}: Reasonable file size ({file_size} bytes)"
                    )
                else:
