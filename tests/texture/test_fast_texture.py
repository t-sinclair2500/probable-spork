...
def test_texture_performance():
    ...
    try:
        ...
        configs = [
            ...
            {
                'name': 'Grain only',
                'config': {'enable': True, 'grain_strength': 0.3, 'feather_px': 0, 'posterize_levels': 1}
            },
            {
                'name': 'Feather only',
                'config': {'enable': True, 'grain_strength': 0, 'feather_px': 2.0, 'posterize_levels': 1}
            },
            {
                'name': 'Posterize only',
                'config': {'enable': True, 'grain_strength': 0, 'feather_px': 0, 'posterize_levels': 4}
            },
            {
                'name': 'All effects',
                'config': {'enable': True, 'grain_strength': 0.3, 'feather_px': 2.0, 'posterize_levels': 4}
            }
        ]
        ...
        for test_config in configs:
            ...
            if test_config['name'] == 'Basic processing':
                ...
                result_img = apply_textures_to_frame(test_img.copy(), test_config['config'], seed=42)
            end_time = time.time()
        ...
        baseline_time = next(r['time_ms'] for r in results if r['name'] == 'No textures')
        ...
        for result in results:
            if result['name'] != 'No textures':
                ...
                print(f"  {result['name']}: {result['time_ms']:.2f}ms (overhead: {overhead:.1f}%)")
