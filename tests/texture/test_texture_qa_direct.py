...
def test_dialback_logic():
    ...
    def _dial_back_texture_strength(config):
        ...
        if "grain_strength" in dialed_config:
            ...
            print(
                f"Reduced grain_strength from {current_strength:.3f} to {dialed_config['grain_strength']:.3f}"
            )
        ...
        if "posterize_levels" in dialed_config:
            ...
            print(
                f"Increased posterize_levels from {current_levels} to {dialed_config['posterize_levels']}"
            )

        # Disable halftone if enabled
        if "halftone" in dialed_config and dialed_config["halftone"].get(
            "enable", False
        ):
        ...
        if "feather_px" in dialed_config:
            ...
            print(
                f"Reduced feather_px from {current_feather:.1f} to {dialed_config['feather_px']:.1f}"
            )
...
def test_qa_loop_logic():
        ...
        while attempts <= max_retries:
            ...
            if contrast_result["ok"]:
                ...
                if attempts <= max_retries:
                    ...
                    print("Max retries reached, using last processed image")
    ...
    # Verify the results
    assert (
        metadata["textures"]["attempts"] == 2
    ), "Should take 2 attempts (fail then pass)"
    assert (
        metadata["textures"]["dialback_applied"] == True
    ), "Dialback should be applied"
...
def main():
    ...
    tests = [test_dialback_logic, test_qa_loop_logic]
