...
class MusicBedPolicyTester:
    ...
    def test_music_selection_criteria(self):
        ...
        for video in self.test_videos:
            ...
            print(
                f"  Mood: {selected_track.mood} (expected: {video['expected_mood']}) - {'✓' if mood_match else '✗'}"
            )

            # Verify BPM matching
            bpm_in_range = (
                video["expected_bpm_range"][0]
                <= selected_track.bpm
                <= video["expected_bpm_range"][1]
            )
            print(
                f"  BPM: {selected_track.bpm} (expected: {video['expected_bpm_range']}) - {'✓' if bpm_in_range else '✗'}"
            )

            # Verify duration compatibility
            duration_ok = selected_track.duration >= video["duration"] * 0.8
            print(
                f"  Duration: {selected_track.duration:.1f}s (video: {video['duration']:.1f}s) - {'✓' if duration_ok else '✗'}"
            )
    ...
    def test_track_library_storage(self):
        ...
        tracks_have_metadata = all(
            track.get("license") and track.get("source") for track in tracks
        )
    ...
    def test_fade_alignment(self):
        ...
        for duration, expected_fade_in, expected_fade_out in test_cases:
            ...
            print(
                f"  Duration {duration}s: fade_in={fade_in_ms}ms, fade_out={fade_out_ms}ms"
            )
            print(
                f"    Expected: fade_in={expected_fade_in}ms, fade_out={expected_fade_out}ms"
            )
    ...
    def test_music_complements_tone(self):
        ...
        for video in self.test_videos:
            ...
            tone_complement = self._check_tone_complement(
                video["tone"], selected_track.mood
            )
    ...
    def test_no_abrupt_cuts(self):
        ...
        # Check loop strategy
        loop_strategy = getattr(
            self.config.music.processing, "loop_strategy", "seamless"
        )
    ...
    def _print_summary(self, results: Dict[str, bool]):
        ...
        if success_rate >= 80:
            print(
                "The music bed policy implementation meets the required success criteria."
            )
        else:
            print(
                "The music bed policy implementation needs improvement to meet success criteria."
            )
