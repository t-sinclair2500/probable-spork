...
class VoiceAdapter:
    ...
    def synthesize_script(
        ...
        final_audio = self._concatenate_audio(
            audio_segments, output_dir, script_path.stem
        )
    ...
    def _synthesize_paragraph(
        self, paragraph: str, index: int, output_dir: Path
    ) -> Path:
    ...
    def _synthesize_with_provider(
        self, provider_name: str, paragraph: str, index: int, output_dir: Path
    ) -> Path:
    ...
    def _synthesize_piper(self, paragraph: str, output_path: Path) -> Path:
        ...
        return provider.synthesize(
            paragraph, output_path, voice_id=voice_id, rate=rate, pitch=pitch, ssml=ssml
        )
    ...
    def _synthesize_openai(self, paragraph: str, output_path: Path) -> Path:
        ...
        response = provider.audio.speech.create(
            model="tts-1", voice=voice, input=paragraph
        )
    ...
    def _concatenate_audio(
        self, audio_segments: List[Path], output_dir: Path, script_name: str
    ) -> Path:
        ...
        for provider_name, provider in self.providers.items():
            try:
                if provider_name == "piper":
                    ...
                    status[provider_name] = {"available": True, "working": True}
                elif provider_name == "openai":
