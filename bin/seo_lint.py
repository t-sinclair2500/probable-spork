...
def lint(title:str, meta_desc:str):
    issues = []
    if len(title) > 65: issues.append("Title >65 chars")
    if len(meta_desc) > 160: issues.append("Meta description >160 chars")
    if not re.match(r"^[a-z0-9-]+$", slugify(title)): issues.append("Slug contains invalid chars after slugify")
    return issues
