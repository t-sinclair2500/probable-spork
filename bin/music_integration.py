...
class MusicIntegrationManager:
    ...
    def prepare_music_for_video(
        ...
        try:
            ...
            if processed_music_path:
                ...
                log_state(
                    "music_integration", "OK", f"Music prepared: {selected_track.title}"
                )
    ...
    def _process_music_for_video(
        self, track: Any, voiceover_path: str, output_dir: str, target_duration: float
    ) -> Optional[str]:
        ...
        try:
            ...
            output_path = os.path.join(
                output_dir, f"bg_processed_{track.title.replace(' ', '_')}.mp3"
            )
            ...
            if track.duration and track.duration < target_duration:
                ...
                if not self.mixer.create_music_loop(
                    track.file_path, output_path, target_duration
                ):
    ...
    def integrate_music_with_video(
        self,
        voiceover_path: str,
        music_path: str,
        output_path: str,
        ...
        try:
            log.info(
                f"Integrating music with voiceover: {voiceover_path} + {music_path}"
            )
            ...
            if success:
                ...
                log_state(
                    "music_integration", "OK", f"Audio mixed with music: {output_path}"
                )
                return True
            else:
    ...
    def create_music_library_from_directory(
        self, source_dir: str, license_info: Dict[str, str] = None
    ) -> int:
        """
                ...
                if audio_file.suffix.lower() in audio_extensions:
                    try:
                        ...
                        track_id = self.library.add_track(
                            ...
                            artist=artist,
                            license=(
                                license_info.get("license", "unknown")
                                if license_info
                                else None
                            ),
                            source=(
                                license_info.get("source", "local")
                                if license_info
                                else None
                            ),
                        )
    ...
    def get_music_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "avg_duration": (
                    sum(t.get("duration", 0) for t in tracks) / len(tracks)
                    if tracks
                    else 0
                ),
            }
...
def test_music_integration():
    ...
    if os.path.exists(test_script) and os.path.exists(test_vo):
        video_metadata = {"tone": "conversational", "duration": 30.0, "pacing_wpm": 165}
