...
try:
    ...
    def resolve_preset_config(preset_name: str, base_config: Dict) -> Dict:
        ...
        log.info(f"Using preset '{normalized_name}': {preset.get('description', 'No description')}")
    ...
    def generate_probe_grid(slug: str, preset_name: Optional[str] = None, 
            ...
            for halftone in halftone_enabled:
                for col_idx, grain in enumerate(grain_strengths):
                    ...
                    try:
                        ...
                        log.warning(f"Failed to apply textures for G:{grain} P:{posterize} H:{halftone}: {e}")
        ...
        if preset_name:
            ...
            if preset_name in presets:
                print(f"Description: {presets[preset_name].get('description', 'No description')}")
...
Examples:
  ...
  python bin/texture_probe.py --slug eames --preset vintage_paper
        ...
        parser.add_argument(
            "--preset",
            help="Style preset name from conf/style_presets.yaml"
        )
        ...
        if args.preset:
            ...
            if normalized_name in presets:
                ...
                print(f"  Grain strength: {preset_config['textures']['grain_strength']}")
                print(f"  Feather pixels: {preset_config['textures']['feather_px']}")
                print(f"  Posterize levels: {preset_config['textures']['posterize_levels']}")
                print(f"  Halftone enabled: {preset_config['textures']['halftone']['enable']}")
