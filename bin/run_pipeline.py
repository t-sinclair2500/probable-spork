...
from bin.utils.config import load_pipeline_config, _read_yaml, load_all_configs
from bin.utils.logs import get_logger, audit_event
...
from bin.contracts.exit_codes import OK as EC_OK, FAIL as EC_FAIL, PARTIAL as EC_PARTIAL, INTERRUPTED as EC_INTR
...
def run_step(step_name: str, cmd: list[str], env: dict | None = None, cwd: str | None = None, notes: str = "", pipeline_cfg = None) -> str:
    """
    ...
    try:
        rc = run_streamed(cmd, cwd=cwd, env=env, log_path=log_path, tail_lines=200, check=True)
        ...
        if required or on_fail == "block":
            audit_event(step_name, "FAIL", error=msg, duration_ms=dur_ms, log_path=log_path)
            log.error(f"[{step_name}] FAIL in {dur_ms} ms: {msg}")
            raise
        elif on_fail == "skip":
            audit_event(step_name, "SKIP", error=msg, duration_ms=dur_ms, log_path=log_path)
            log.warning(f"[{step_name}] SKIP (optional) in {dur_ms} ms: {msg}")
            return "SKIP"
        else:
            # warn
            audit_event(step_name, "PARTIAL", error=msg, duration_ms=dur_ms, log_path=log_path)
            log.warning(f"[{step_name}] PARTIAL (optional; continuing) in {dur_ms} ms: {msg}")
            return "PARTIAL"
...
def _should_run_shared_ingestion(args) -> bool:
    ...
    """
    return not getattr(args, "yt_only", False)
...
def _run_minimal_orchestrator(argv: Optional[list] = None) -> int:
    ...
    cfg = load_all_configs(profile=args.profile, cli_overrides={("research"): {"policy": {"mode": args.mode}}} if args.mode else None)
    ...
    steps = [
        Step("research_collect", [sys.executable, os.path.join(BASE, "bin", "research_collect.py"), "--slug", slug],
             required=pol.get("research_collect", {}).get("required", True) if isinstance(pol, dict) else True,
             on_fail=pol.get("research_collect", {}).get("on_fail", "block") if isinstance(pol, dict) else "block",
             idempotent_outputs=[str(apaths["research_sources"])]),
        Step("research_ground", [sys.executable, os.path.join(BASE, "bin", "research_ground.py"), "--slug", slug],
             required=pol.get("research_ground", {}).get("required", True) if isinstance(pol, dict) else True,
             on_fail=pol.get("research_ground", {}).get("on_fail", "block") if isinstance(pol, dict) else "block",
             idempotent_outputs=[str(apaths["grounded_beats"]), str(apaths["references"])]),
        Step("storyboard", [sys.executable, os.path.join(BASE, "bin", "storyboard_plan.py"), "--slug", slug],
             required=pol.get("storyboard", {}).get("required", True) if isinstance(pol, dict) else True,
             idempotent_outputs=[str(apaths["scenescript"])]),
        Step("animatics", [sys.executable, os.path.join(BASE, "bin", "animatics_generate.py"), "--slug", slug],
             required=pol.get("animatics", {}).get("required", True) if isinstance(pol, dict) else True,
             idempotent_outputs=[str(apaths["animatics_dir"])]),
        Step("audio", [sys.executable, os.path.join(BASE, "bin", "tts_generate.py"), "--slug", slug],
             required=pol.get("audio", {}).get("required", True) if isinstance(pol, dict) else True,
             idempotent_outputs=[str(apaths["voiceover_mp3"]), str(apaths["voiceover_srt"])]),
        Step("assemble", [sys.executable, os.path.join(BASE, "bin", "assemble_video.py"), "--slug", slug],
             required=pol.get("assemble", {}).get("required", True) if isinstance(pol, dict) else True,
    ...
    def _runner(step_name: str, cmd: list[str]) -> str:
        log_path = os.path.join("logs", "subprocess", f"{step_name}.log")
        try:
            rc = run_streamed(cmd, cwd=BASE, env=os.environ.copy(), log_path=log_path, tail_lines=200, check=True)
...
def run_step_legacy(script_name: str, args: List[str] = None, required: bool = True, brief_env: Dict[str, str] = None, brief_data: Dict[str, any] = None, models_config: Dict = None, pipeline_cfg = None) -> bool:
    """
    ...
    try:
        ...
        try:
            ...
            if rc == 0:
                ...
                log_state(script_name, "FAIL", f"exit_code={rc};elapsed_ms={elapsed_ms}")
            ...
            elapsed_ms = int((time.time() - start_time) * 1000)
            log_state(script_name, "FAIL", f"exception={str(e)};elapsed_ms={elapsed_ms}")
...
def _run_llm_step(script_name: str, model_name: str, args: List[str] = None, required: bool = True, brief_env: Dict[str, str] = None, brief_data: Dict[str, any] = None, pipeline_cfg = None) -> bool:
    ...
    try:
        ...
        with model_session(model_name) as session:
            ...
            # Run the actual step
            return _run_subprocess_step(script_name, args, required, brief_env, brief_data, pipeline_cfg)
...
def _run_subprocess_step(script_name: str, args: List[str] = None, required: bool = True, brief_env: Dict[str, str] = None, brief_data: Dict[str, any] = None, pipeline_cfg = None) -> bool:
    ...
    try:
        status = run_step(script_name, cmd, env=env, cwd=BASE, notes=f"subprocess step", pipeline_cfg=pipeline_cfg)
        return status == "OK"
    except Exception as e:
...
def run_youtube_lane(cfg, dry_run: bool = False, brief_env: Dict[str, str] = None, brief_data: Dict[str, any] = None, models_config: Dict = None) -> bool:
    ...
    video_steps_raw = pipeline_cfg.get("execution", {}).get("video_production", [
        "tts_generate", "generate_captions", "assemble_video", "make_thumbnail", "upload_stage"
    ])
    ...
    for step in video_steps_raw:
        if step == "generate_captions":
            video_steps.append((step, False))  # Optional - graceful skip if whisper.cpp missing
        else:
            video_steps.append((step, True))
    
    for step_name, required in video_steps:
        step_success = run_step_legacy(step_name, required=required, brief_env=brief_env, brief_data=brief_data)
...
def run_shared_ingestion(cfg, from_step: Optional[str] = None, brief_env: Optional[Dict[str, str]] = None, brief_data: Optional[Dict] = None, models_config: Optional[Dict] = None, no_style_rewrite: bool = False) -> bool:
    ...
    if hasattr(pipeline_cfg, 'execution'):
        shared_steps = getattr(pipeline_cfg.execution, 'shared_ingestion', [
        ...
        for step_name, required in batch["steps"]:
            ...
            if step_name == "research_ground" and slug:
                ...
                if not script_files:
                    ...
                    step_success = run_step_legacy(step_name, args=[latest_script, "--slug", slug], required=required, brief_env=brief_env, brief_data=brief_data, models_config=models_config)
                    if not step_success and required:
                        batch_success = False
            ...
            if step_name == "research_collect" and slug:
                step_success = run_step_legacy(step_name, args=["--slug", slug], required=required, brief_env=brief_env, brief_data=brief_data, models_config=models_config)
            # Special handling for fact_check - pass script path
                ...
                if not script_files:
                    ...
                    step_success = run_step_legacy(step_name, args=[latest_script], required=required, brief_env=brief_env, brief_data=brief_data, models_config=models_config)
                    if not step_success and required:
                        batch_success = False
                        log.error(f"Required step failed: {step_name}")
                        break
                ...
                step_success = run_step_legacy(step_name, required=required, brief_env=brief_env, brief_data=brief_data, models_config=models_config)
            if not step_success and required:
                batch_success = False
                log.error(f"Required step failed: {step_name}")
                break
            ...
            if not script_files:
                ...
                for step_name in storyboard_steps:
                    if step_name == "storyboard_plan":
                        step_success = run_step_legacy(step_name, args=["--slug", slug], required=True, brief_env=brief_env, brief_data=brief_data, models_config=models_config)
                    elif step_name == "animatics_generate":
                        step_success = run_step_legacy(step_name, args=["--slug", slug], required=True, brief_env=brief_env, brief_data=brief_data, models_config=models_config)
                    else:
                        step_success = run_step_legacy(step_name, required=True, brief_env=brief_env, brief_data=brief_data, models_config=models_config)
...
def main():
    ...
    parser.add_argument("--profile", choices=["m2_8gb_optimized", "pi_8gb"], 
                       default=None, help="Platform profile overlay")
