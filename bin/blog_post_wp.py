...
def wp_auth(bcfg):
    import requests
    return (bcfg["wordpress"]["base_url"].rstrip("/"),
            requests.auth.HTTPBasicAuth(bcfg["wordpress"]["api_user"], bcfg["wordpress"]["api_app_password"]))
...
def ensure_category(base, auth, name):
    ...
    if r.ok and r.json():
        return r.json()[0]["id"]
    r = requests.post(f"{base}/wp-json/wp/v2/categories", auth=auth, json={"name":name}, timeout=30)
    r.raise_for_status()
    return r.json()["id"]
...
def recent_posts(base, auth, n=10):
    import requests
    r = requests.get(f"{base}/wp-json/wp/v2/posts?per_page={n}&_fields=link,title", auth=auth, timeout=30)
    if not r.ok: return []
    return r.json()
...
def main():
    ...
    bcfg = load_blog_cfg()
    meta = json.load(open(os.path.join(BASE,"data","cache","post.meta.json"),"r",encoding="utf-8"))
    ...
    try:
        ...
        if posts:
            links = "".join([f'<li><a href="{p["link"]}">{p["title"]["rendered"]}</a></li>' for p in posts])
