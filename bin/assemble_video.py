...
def main():
    ...
    if not files:
        log_state("assemble_video","SKIP","no scripts"); print("No scripts"); return
    files.sort(reverse=True)
    key = files[0].replace(".txt","")
    vo = os.path.join(vodir, key + ".mp3")
    if not os.path.exists(vo):
        log_state("assemble_video","FAIL","no voiceover"); print("No VO"); return
    ...
    import subprocess
    probe = subprocess.run(f'ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 "{vo}"',
