...
from bin.core import BASE, get_logger, load_config, log_state, parse_llm_json
...
def call_fact_checker(content: str, cfg, models_config=None) -> Dict:
    ...
    try:
        ...
        payload = {"model": cfg.llm.model, "prompt": full_prompt, "stream": False}
...
def fact_check_content(markdown_content: str, config=None) -> Dict:
    ...
    # Log results
    log.info(
        f"Fact-check completed in {processing_time:.2f}s: "
...
def should_gate_content(
    fact_check_result: Dict, gate_mode: str, severity_threshold: str = "warning"
) -> bool:
    """
...
def main(brief=None):
    ...
    parser.add_argument(
        ...
        default="warning",
        help="Minimum severity for gating",
    )
    ...
    if brief:
        ...
        if brief_keywords:
            print(
                f"Brief keywords for fact-checking priority: {', '.join(brief_keywords)}"
            )
            ...
            print(
                f"Prioritized {len([l for l in prioritized_lines if l.startswith('PRIORITY:')])} lines for fact-checking"
            )
    ...
    if brief:
        brief_title = brief.get("title", "Untitled")
        log_state(
            "fact_check", "OK", f"issues={metrics['total_issues']};brief={brief_title}"
        )
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "input_file", nargs="?", help="Markdown file to fact-check (optional)"
    )
