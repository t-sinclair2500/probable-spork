...
from bin.core import BASE, get_logger, load_config, sha1_file, slugify  # noqa: E402
...
class FixtureManager:
    ...
    def __init__(self, config):
        ...
        if hasattr(config, 'testing'):
            self.testing_cfg = getattr(config.testing, '__dict__', {}) if hasattr(config.testing, '__dict__') else {}
        else:
            self.testing_cfg = config.get("testing", {}) if hasattr(config, 'get') else {}
        self.fixture_dir = os.path.join(BASE, self.testing_cfg.get("fixture_dir", "assets/fixtures"))
    ...
    def copy_from_slug(self, slug: str) -> int:
        ...
        for file_name in os.listdir(slug_dir):
            ...
            if file_hash in existing_hashes:
                log.info(f"Asset {file_name} already exists in fixtures (hash: {file_hash[:8]})")
                continue
    ...
    def _update_fixtures_license(self, source_license: Dict):
        ...
        source_info = {
            ...
            "providers": list(set(asset.get("provider", "unknown") for asset in source_license.get("assets", []))),
        ...
        # Update timestamp
        import time
        fixtures_license["created_at"] = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
...
def main():
    parser = argparse.ArgumentParser(description="Prepare test fixtures for asset testing")
    parser.add_argument("--from-slug", type=str, help="Copy assets from specific slug directory")
    parser.add_argument("--make-synthetic", type=int, help="Create N synthetic test images")
    parser.add_argument("--list", action="store_true", help="List current fixture inventory")
