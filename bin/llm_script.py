...
def enforce_cta_policy(script_text, cta_policy, intent):
    ...
    if cta_policy == "omit":
        ...
        for pattern in cta_patterns:
            script_text = re.sub(pattern, '[CTA REMOVED]', script_text, flags=re.IGNORECASE)
        
        return script_text, "omitted"
        
    elif cta_policy == "require":
        # Ensure CTA is present
        if not any(word in script_text.lower() for word in ['subscribe', 'like', 'comment', 'share', 'follow']):
        ...
        if not any(word in script_text.lower() for word in ['recommend', 'suggest', 'best', 'top', 'choice']):
...
def generate_script(outline_path: str, target_len_sec: int = 60, brief: Dict = None, 
    ...
    try:
        ...
        try:
            ...
            log.warning(f"[script] Failed to load intent template '{outline_intent}': {e}")
            intent_template = None
            intent_metadata = None
        ...
        if intent_template:
            try:
                ...
                log.info(f"[script] Citation requirements: {citation_reqs['beats_needing_citations']}/{citation_reqs['total_beats']} beats need citations")
            except Exception as e:
        ...
        evidence_load = intent_metadata['evidence_load'] if intent_metadata else evidence_load
        ...
        with model_session(model_name) as session:
            ...
            try:
                ...
                data = create_fallback_script(outline_data, target_len_sec, outline_intent)
        ...
        script_text, cta_action = enforce_cta_policy(script_text, cta_policy, outline_intent)
        ...
        log_state("llm_script", "OK", f"{os.path.basename(outline_path)}.txt;intent={outline_intent};cta_action={cta_action}")
...
def create_fallback_script(outline_data: Dict, target_len_sec: int, intent: str) -> Dict:
    """Create a fallback script when LLM generation fails."""
    log.warning(f"[script] Creating fallback script for {outline_data.get('topic', 'Unknown')}")
    
    sections = outline_data.get('sections', [])
    script_parts = []
    
    for section in sections:
        section_content = f"[{section.get('title', 'Section')} content will be generated]"
    ...
    return {
        ...
        'metadata': {
            ...
            'citation_placeholders_count': sum(1 for s in sections if s.get('needs_citations', True))
        }
    }


def validate_and_enhance_script(data: Dict, outline_data: Dict, target_len_sec: int) -> Dict:
...
def insert_citation_placeholders(data: Dict, outline_data: Dict) -> Dict:
    ...
    for script_section in sections:
        ...
        for outline_sec in outline_sections:
            if (outline_sec.get('id') == script_section.get('id') or 
                outline_sec.get('title') == script_section.get('title')):
        ...
        if outline_section and outline_section.get('needs_citations', True):
            ...
            script_section['citation_placeholders'] = ["[CITATION NEEDED] for factual claims"]
...
def main(brief=None, models_config=None, outline_path=None, slug=None):
    ...
    parser.add_argument("--slug", required=True, help="Topic slug for script generation")
    ...
    log.info(f"[script] Starting script generation for slug: {args.slug}, outline: {outline_path}, duration: {args.duration}s")
