...
def rasterize_element_assets(
    elements: List[Element], style: BrandStyle, texture_config: Optional[Dict] = None
) -> Dict[str, str]:
                ...
                if element.asset_path.endswith(".svg"):
                    ...
                    if raster_path:
                        ...
                        if texture_config and texture_config.get("enable", False):
                            try:
                                from .cutout.texture_integration import (
                                    process_rasterized_with_texture_qa,
                                )

                                textured_path, metadata = (
                                    process_rasterized_with_texture_qa(
                                ...
                                if textured_path != raster_path:
                                    ...
                                    log.info(
                                        f"[texture-integrate] Applied texture to {element.asset_path} -> {textured_path}"
                                    )
                                else:
                                    asset_paths[element.id] = raster_path
                                    log.debug(
                                        f"Rasterized {element.asset_path} -> {raster_path}"
                                    )
                            except ImportError:
                                log.warning(
                                    "[texture-integrate] Texture integration not available, using untextured version"
                                )
                                asset_paths[element.id] = raster_path
                        else:
                            asset_paths[element.id] = raster_path
    ...
    if texture_metadata:
        log.info(
            f"[texture-integrate] Texture metadata collected for {len(texture_metadata)} elements"
        )
...
def create_texture_metadata(texture_config: Dict, texture_results: Dict) -> Dict:
    ...
    if texture_results:
        ...
        successful_applications = sum(
            1
            for r in texture_results.values()
            if r.get("textures", {}).get("applied", False)
        )
        dialback_applied = sum(
            1
            for r in texture_results.values()
            if r.get("textures", {}).get("dialback_applied", False)
        )
        ...
        metadata["textures"]["summary"] = {
            ...
            "success_rate": (
                successful_applications / total_elements if total_elements > 0 else 0.0
            ),
        }
        ...
        if qa_attempts:
            metadata["textures"]["qa_summary"] = {
                ...
                "average_attempts_per_element": (
                    sum(qa["attempt"] for qa in qa_attempts) / len(texture_results)
                    if texture_results
                    else 0
                ),
                "max_attempts": (
                    max(qa["attempt"] for qa in qa_attempts) if qa_attempts else 0
                ),
            }
...
def write_animatics_metadata(
    slug: str, texture_metadata: Dict, texture_config: Dict
) -> str:
    ...
    log.info(
        f"[texture-integrate] Wrote animatics metadata with texture info: {metadata_path}"
    )
...
def render_scene(
    ...
    try:
            ...
            try:
                ...
                cfg_dict = {
                    "procedural": {
                        ...
                        "placement": {
                            "min_spacing_px": (
                                getattr(procedural_cfg.placement, "min_spacing_px", 64)
                                if procedural_cfg.placement
                                else 64
                            ),
                            "safe_margin_px": (
                                getattr(procedural_cfg.placement, "safe_margin_px", 40)
                                if procedural_cfg.placement
                                else 40
                            ),
                        },
                        "layout": {
                            ...
                            "prefer_thirds": (
                                getattr(procedural_cfg.layout, "prefer_thirds", True)
                                if procedural_cfg.layout
                                else True
                            ),
                            "max_attempts": (
                ...
                log.info(
                    f"[layout] Auto-placed {sum(1 for e in scene.elements if 'x' in e and 'y' in e)} elements (seed={scene_seed})"
                )

                # Validate layout
                if not check_scene_layout_validity(scene_dict):
                    log.warning(
                        f"[layout] Scene {scene.id} has layout validation issues"
                    )
        ...
        log.info(
            f"[duration-policy] Rendering scene {scene.id} with duration: {scene.duration_ms}ms"
        )
        ...
        if scene.bg:
            ...
            try:
                ...
                bg_svg = generate_background_motif(
                    scene.bg, scene_colors, seed=hash(scene.id) % 10000
                )
                ...
                log.warning(
                    f"Procedural background generation failed for scene {scene.id}: {e}"
                )
                # Fallback to brand assets
                bg_path = get_background_path(scene.bg, style)
                if bg_path:
        ...
        try:
            ...
            if modules_config_path.exists():
                ...
                if micro_anim_config.get("enable", False):
                    ...
                    micro_anim_gen = create_micro_animation_generator(
                        micro_anim_config, seed=scene_seed
                    )
                    ...
                    if (
                        animation_result["enabled"]
                        and animation_result["elements_animated"] > 0
                    ):
                        log.info(
                            f"[micro-anim] Applied animations to {animation_result['elements_animated']} elements in scene {scene.id}"
                        )
                        ...
                        log.debug(
                            f"[micro-anim] No elements animated in scene {scene.id}"
                        )
            ...
            log.warning(
                f"[micro-anim] Failed to generate micro-animations for scene {scene.id}: {e}"
            )

        # Create element clips
        for element in scene.elements:
            element_clip = create_element_clip(
                element, style, asset_paths, scene_duration
            )
            if element_clip:
...
def render_animatics(slug: str, scene_id: Optional[str] = None) -> bool:
    ...
    try:
        ...
        texture_metadata = {}
        try:
            if procedural_cfg and procedural_cfg.get("textures", {}).get(
                "enable", False
            ):
        ...
        for i, scene in enumerate(scenes_to_render):
            ...
            if output_path.exists():
                print("   ⏭️  Already rendered, skipping")
        ...
        print("\n" + "=" * 50)
        print(
            f"🎉 Rendering Complete: {successful_renders}/{total_scenes} scenes ({completion_rate:.1f}%)"
        )
        ...
        if texture_metadata:
            try:
                metadata_path = write_animatics_metadata(
                    slug, texture_metadata, texture_config
                )
                log.info(
                    f"[texture-integrate] Wrote texture metadata to {metadata_path}"
                )
            except Exception as e:
