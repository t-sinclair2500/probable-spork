...
class ResearchGrounder:
    ...
    def __init__(self, models_config: Optional[Dict] = None):
        ...
        # Load domain scores from config
        research_data = (
            bundle.get("research", {})
            if isinstance(bundle, dict)
            else bundle.research.dict()
        )
        self.domain_scores = research_data.get("domain_scores", {}) or {}
        ...
        self.target_citations_per_beat = (
            self.policy.min_citations * 2
        )  # Target 2x minimum
        self.min_beats_coverage_pct = (
            getattr(self.policy, "coverage_threshold", 0.65) * 100
        )  # Convert to percentage
        self.citation_format = "[S{num}]"  # Default format

        # Quality thresholds from policy
        self.min_domain_quality_score = getattr(self.policy, "min_domain_score", 0.6)
        self.min_content_relevance_score = 0.7  # Default

    def _score_domain(self, url: str) -> float:
        """Score a domain based on configured domain scores."""
    ...
    def ground_script(self, script_path: Path, brief: Dict) -> Dict:
        ...
        for i, beat in enumerate(beats):
            log.info(
                f"[ground] Grounding beat {i+1}/{len(beats)}: {beat['title'][:50]}..."
            )
        ...
        quality_report = self._validate_grounding_quality(
            grounded_beats, normalized_references
        )
    ...
    def _ground_beat(self, beat: Dict, brief: Dict) -> Dict:
        ...
        if not chunks:
            log.warning(
                f"[ground] No relevant research found for beat: {beat['title']}"
            )
            return {
                **beat,
        ...
        selected_chunks = self._select_best_chunks(
            scored_chunks, self.target_citations_per_beat
        )

        # Ground content with selected research
        grounded_content = self._ground_content_with_research(
            beat["content"], selected_chunks
        )
        ...
        return {
            **beat,
            ...
            "citations": citations,
            "research_score": (
                sum(chunk.get("score", 0) for chunk in selected_chunks)
                / len(selected_chunks)
                if selected_chunks
                else 0.0
            ),
    ...
    def _calculate_domain_quality_score(self, domain: str) -> float:
        """Calculate domain quality score based on configured domain scores."""
        # Use configured domain scores from research.yaml
        return self.domain_scores.get(domain.lower(), 0.5)

    def _select_best_chunks(
        self, scored_chunks: List[Dict], target_count: int
    ) -> List[Dict]:
        ...
        qualified_chunks = [
            ...
            if chunk["score"] >= self.min_domain_quality_score
        ]
        ...
        log.info(
            f"[ground] Selected {len(selected)} chunks from {len(scored_chunks)} candidates"
        )
        return selected
...
Enhanced Content:"""
        ...
        try:
            ...
            models_data = (
                bundle.get("models", {})
                if isinstance(bundle, dict)
                else bundle.models.dict()
            )
            model_name = models_data.get("defaults", {}).get(
    ...
    def _validate_grounding_quality(
        self, grounded_beats: List[Dict], references: List[Dict]
    ) -> Dict:
        """Validate grounding quality against requirements."""
        total_beats = len(grounded_beats)
        beats_with_citations = sum(
            1 for beat in grounded_beats if beat.get("citations")
        )
        ...
        avg_citations = (
            sum(len(beat.get("citations", [])) for beat in grounded_beats) / total_beats
            if total_beats > 0
            else 0
        )
        ...
        quality_report = {
            ...
            "overall_quality": (
                "PASS" if (meets_coverage and meets_citation_min) else "FAIL"
            ),
            "quality_issues": [],
        }
    ...
    def _generate_grounded_script(
        self, original_script: str, grounded_beats: List[Dict]
    ) -> str:
        ...
        for beat in grounded_beats:
            ...
            if beat["original_content"] in grounded_script:
                grounded_script = grounded_script.replace(
                    beat["original_content"], beat["grounded_content"]
                )

        return grounded_script

    def _save_grounded_results(
        self, output_dir: Path, grounded_beats: List[Dict], references: List[Dict]
    ):
...
def main(brief=None, models_config=None, script_path=None, slug=None):
    ...
    parser.add_argument(
        "--slug", required=True, help="Topic slug for research grounding"
    )
    ...
    log.info(
        f"[ground] Starting research grounding for script: {script_path}, slug: {slug}"
    )
    ...
    log.info("[ground] Research grounding completed")
