...
#!/usr/bin/env python3
import os, subprocess, json, time, re, shlex, argparse, sys
# Ensure repo root is on sys.path for `import bin.core`
ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
if ROOT not in sys.path:
    sys.path.insert(0, ROOT)
from bin.core import single_lock, log_state, load_config, BASE, get_logger
...
def main():
    ...
    if args.input:
        ...
        if not files:
            log_state("generate_captions","SKIP","no voiceovers"); print("No voiceovers"); return
    ...
    model_path = cfg.asr.model if os.path.isabs(cfg.asr.model) else os.path.join(os.path.expanduser("~"), "whisper.cpp", "models", cfg.asr.model)
