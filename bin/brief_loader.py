...
def _normalize_brief_fields(brief: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Normalize additional list fields that might be present
...
def create_brief_template(output_path: str = None) -> str:
    ...
    template = {
        ...
        "keywords_exclude": ["AI tools", "crypto", "NFT"],
        "sources_preferred": ["Google Search Central", "Sterling Sky"],
        "monetization": {
            "primary": ["lead_magnet", "email_capture"],
...
def _enhance_brief_with_llm(
    text: str, base_brief: Dict[str, Any]
    ...
    try:
        ...
        prompt = template.format(
            brief_context="", text=text  # Brief enhancement doesn't use brief context
        )
        ...
        if llm_result.returncode == 0:
            ...
            if json_match:
                enhanced = yaml.safe_load(
                    json_match.group()
                )  # Use yaml.safe_load for JSON-like YAML
