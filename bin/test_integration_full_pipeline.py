...
def test_legacy_removal():
    ...
    try:
        ...
        if fetch_assets_path.exists():
            print("✗ fetch_assets.py still exists - should be removed")
            return False

        legacy_dir = Path("legacy")
        if legacy_dir.exists():
            print("✗ legacy directory still exists - should be removed")
            return False
...
def test_design_prompt_integration():
    ...
    try:
        ...
        if not design_animatics_dir.exists():
            print(
                "⚠ Design animatics directory not found, may need to run animatics generation"
            )
        ...
        if design_coverage.exists():
            ...
            print("⚠ Design asset coverage report not found")
...
def main():
    ...
    if passed == total:
        ...
        print(
            f"⚠ {total - passed} tests failed - review integration before E2E testing"
        )
        return 1
