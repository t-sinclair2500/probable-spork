...
class LegibilityValidator:
    ...
    def _load_safe_backgrounds(self) -> List[str]:
        ...
        try:
            ...
            if os.path.exists(design_language_path):
                ...
                if backgrounds:
                    log.info(f"[legibility-defaults] Loaded {len(backgrounds)} safe background colors")
                    return backgrounds
                else:
                    log.warning("[legibility-defaults] No background colors found in design language")
            else:
        ...
        log.info(f"[legibility-defaults] Using {len(fallback_backgrounds)} fallback background colors")
        return fallback_backgrounds
    
    def validate_contrast_for_acceptance(self, text_color: str, background_color: str, 
                                       element_id: str = "unknown") -> Dict[str, Any]:
        """
        ...
        """
        log.info(f"[legibility-defaults] Validating contrast for {element_id}: {text_color} on {background_color}")
        ...
        try:
            ...
            log.info(f"[legibility-defaults] Contrast validation: {contrast_ratio:.2f} "
                    f"({'PASS' if wcag_aa_pass else 'FAIL'}) for {element_id}")
            
            return result
            
        except Exception as e:
            log.error(f"[legibility-defaults] Contrast validation error for {element_id}: {str(e)}")
            return {
                "valid": False,
    ...
    def inject_safe_background(self, text_color: str, element_id: str = "unknown") -> Dict[str, Any]:
        """
        ...
        """
        log.info(f"[legibility-defaults] Injecting safe background for {element_id} with text color {text_color}")
        ...
        try:
            ...
            validation = self.validate_contrast_for_acceptance(text_color, best_background, element_id)
            ...
            log.info(f"[legibility-defaults] Background injection successful: {best_background} "
                    f"for {element_id} (contrast: {validation['contrast_ratio']:.2f})")
            
            return result
            
        except Exception as e:
            log.error(f"[legibility-defaults] Background injection failed for {element_id}: {str(e)}")
            return {
                "success": False,
    ...
    def _find_best_background(self, text_color: str) -> Optional[str]:
        ...
        try:
            ...
            if best_contrast >= self.wcag_aa_threshold:
                ...
                log.warning(f"[legibility-defaults] Best contrast {best_contrast:.2f} below threshold "
    ...
    def _generate_contrast_recommendations(self, text_color: str, background_color: str, 
                                         current_contrast: float) -> List[str]:
        """Generate recommendations for improving contrast"""
        recommendations = []
        
        if current_contrast < self.wcag_aa_threshold:
            recommendations.append(f"Current contrast {current_contrast:.2f} is below WCAG-AA threshold {self.wcag_aa_threshold}")
            
            # Suggest background injection
            best_bg = self._find_best_background(text_color)
            if best_bg:
                recommendations.append(f"Consider using background color {best_bg} for better contrast")
            
            # General recommendations
            if current_contrast < 3.0:
                recommendations.append("Consider using darker text or lighter background")
            else:
                recommendations.append("Consider using slightly darker text or lighter background")
            
            # Check if we can suggest a better text color
            if background_color in self.safe_backgrounds:
                recommendations.append("Current background is in safe palette - consider adjusting text color")
        
        return recommendations
    
    def validate_scenescript_legibility(self, scenescript_data: Dict[str, Any]) -> Dict[str, Any]:
        """
            ...
            for scene_idx, scene in enumerate(scenes):
                ...
                for element_idx, element in enumerate(elements):
                    if element.get("type") == "text":
                        element_id = element.get("element_id", f"{scene_id}_element_{element_idx}")
                        ...
                        if not background_color:
                            ...
                            injection_result = self.inject_safe_background(text_color, element_id)
                            if injection_result["success"]:
                                overall_results["background_injections"].append(injection_result)
                                background_color = injection_result["injected_background"]
                                log.info(f"[legibility-defaults] Injected background {background_color} for {element_id}")
                            else:
                        ...
                        if not background_color and contrast_result["valid"]:
                            ...
                            log.warning(f"[legibility-defaults] Element {element_id} passed without background")
            
            # Summary
            if overall_results["failed_elements"] > 0:
                overall_results["summary"] = f"{overall_results['failed_elements']} of {overall_results['validated_elements']} elements failed contrast validation"
            else:
                overall_results["summary"] = f"All {overall_results['validated_elements']} elements passed contrast validation"
            
            log.info(f"[legibility-defaults] SceneScript validation completed: "
...
def validate_contrast_for_acceptance(text_color: str, background_color: str, 
                                   element_id: str = "unknown") -> Dict[str, Any]:
    """Convenience function for acceptance pipeline"""
    validator = LegibilityValidator()
    return validator.validate_contrast_for_acceptance(text_color, background_color, element_id)


def inject_safe_background(text_color: str, element_id: str = "unknown") -> Dict[str, Any]:
    """Convenience function for acceptance pipeline"""
    validator = LegibilityValidator()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Validate text legibility and inject backgrounds")
    parser.add_argument("--text-color", required=True, help="Text color in hex format")
    parser.add_argument("--background-color", help="Background color in hex format (optional)")
