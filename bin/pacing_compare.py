...
try:
    from .core import get_logger, load_brief, load_modules_cfg
except ImportError:
...
def get_current_intent() -> str:
    ...
    try:
        ...
        try:
            ...
            log.warning(
                f"[pacing-compare] Could not load brief config, using default intent: {e}"
            )
            return "default"
...
def compare_to_profile(kpi_data: Dict, profile: Dict) -> Dict:
    ...
    log.info(
        f"[pacing-compare] Comparison complete: {overall_status} overall, "
...
def _compare_metric(value: float, band: List[float], metric_name: str) -> str:
    ...
    if value < min_val:
        ...
        log.debug(
            f"[pacing-compare] {metric_name}: {value:.2f} in range [{min_val:.2f}, {max_val:.2f}] (ok)"
        )
        return "ok"
...
def save_pacing_report(
    slug: str, kpi_data: Dict, comparison: Dict, report_dir: str = None
) -> str:
    """
...
def update_video_metadata(
    slug: str, kpi_data: Dict, comparison: Dict, metadata_dir: str = None
) -> str:
    """
    ...
    log.info(
        f"[pacing-compare] Video metadata updated with pacing comparison: {metadata_path}"
    )
...
def run_pacing_analysis(slug: str, kpi_data: Dict) -> Dict:
    ...
    if os.path.exists(scenescript_path):
        ...
        adjusted_scenes, feedback_summary = _run_integrated_feedback_adjustment(
            slug, feedback_data, scenes, modules_cfg
        )
        feedback_result = {
        ...
        if feedback_summary.get("adjusted"):
            log.info(
                f"[pacing-compare] Applied adjustments to {feedback_summary['adjusted_scenes']} scenes"
            )

            # Save adjusted scenescript
            adjusted_path = os.path.join(
                os.path.dirname(__file__), "..", "scenescripts", f"{slug}_adjusted.json"
            )
            ...
            adjusted_scenescript["metadata"]["pacing_adjustment_ms"] = feedback_summary[
                "total_adjustment_ms"
            ]
            ...
            if os.path.exists(metadata_path):
                ...
                metadata["pacing"]["adjustments_applied"] = feedback_summary[
                    "total_adjustment_ms"
                ]
                ...
                log.info(
                    f"[pacing-compare] Updated metadata: adjusted=true, total_adjustment={feedback_summary['total_adjustment_ms']}ms"
                )
        else:
...
def _check_adjustment_need(kpi: Dict, bands: Dict, tolerance_pct: float) -> bool:
    ...
    log.info(
        f"[pacing-compare] Checking adjustment need with tolerance: {tolerance_pct}%"
    )
    ...
    for metric in metrics:
        ...
        if min_val <= value <= max_val:
            log.debug(
                f"[pacing-compare] {metric} {value} within band [{min_val}, {max_val}]"
            )
            continue
        ...
        if value < min_val:
            ...
            log.info(
                f"[pacing-compare] {metric} {value} below band [{min_val}, {max_val}], "
                f"deviation: {deviation:.1f}%"
            )
        else:
            deviation = (value - max_val) / max_val * 100
            log.info(
                f"[pacing-compare] {metric} {value} above band [{min_val}, {max_val}], "
                f"deviation: {deviation:.1f}%"
            )

        if deviation > tolerance_pct:
            log.info(
                f"[pacing-compare] {metric} {value} outside band [{min_val}, {max_val}], "
                f"deviation: {deviation:.1f}% > {tolerance_pct}% - ADJUSTMENT NEEDED"
            )
            return True
        else:
            log.info(
                f"[pacing-compare] {metric} {value} outside band but within tolerance: "
...
def _calculate_metric_adjustments(
    ...
    log.info("[pacing-compare] Calculating metric adjustments...")
    ...
    for metric in ["words_per_sec", "cuts_per_min", "avg_scene_s"]:
        ...
        if value < min_val:
            ...
            log.info(
                f"[pacing-compare] {metric} below band: {value} < {min_val} -> +1.0"
            )
        elif value > max_val:
            # Value is too high, need to decrease (shorter scenes, faster pacing)
            adjustments[metric] = -1.0  # Negative adjustment
            log.info(
                f"[pacing-compare] {metric} above band: {value} > {max_val} -> -1.0"
            )
        else:
            adjustments[metric] = 0.0  # No adjustment needed
            log.info(
                f"[pacing-compare] {metric} within band: {min_val} <= {value} <= {max_val} -> 0.0"
            )
...
def _calculate_scene_slack(scenes: List[Dict], cfg: Dict) -> Dict[str, int]:
    ...
    for scene in scenes:
        ...
        log.debug(
            f"[pacing-compare] Scene {scene_id}: duration={duration}ms, "
...
def _generate_scene_adjustments(
    ...
    for scene_id, slack, scene in scenes_with_slack:
        ...
        log.debug(
            f"[pacing-compare] Scene {scene_id}: slack={slack}ms, "
...
def _apply_scene_adjustments(
    ...
    for i, scene in enumerate(scenes):
        ...
        if actual_delta != delta_ms:
            log.info(
                f"[pacing-compare] Scene {scene_id} adjustment clamped: "
        ...
        log.info(
            f"[pacing-compare] Scene {scene_id}: {original_duration}ms → {clamped_duration}ms "
            f"(delta: {actual_delta:+d}ms)"
        )

    log.info(
        f"[pacing-compare] Applied adjustments to {len(adjusted_scenes)} scenes, "
...
def _run_integrated_feedback_adjustment(
    ...
    log.info(
        f"[pacing-compare] Integrated feedback adjustment complete: {summary['adjusted_scenes']} scenes adjusted"
    )
        ...
        if "kpi_metrics" in report_data:
            ...
            if force_feedback:
                ...
                if "kpi_metrics" in kpi_data_for_feedback:
                    ...
                    print(
                        f"Extracted KPI metrics: {list(kpi_data_for_feedback.keys())}"
                    )
                ...
                if result.get("feedback_result"):
                    ...
                    feedback_adjusted = feedback.get("adjusted") or feedback.get(
                        "summary", {}
                    ).get("adjusted", False)
                    if feedback_adjusted:
                        adjusted_scenes = feedback.get(
                            "adjusted_scenes"
                        ) or feedback.get("summary", {}).get("adjusted_scenes", 0)
                        total_adjustment = feedback.get(
                            "total_adjustment_ms"
                        ) or feedback.get("summary", {}).get("total_adjustment_ms", 0)
                        print(
                            f"Feedback adjustment applied: {adjusted_scenes} scenes adjusted"
                        )
