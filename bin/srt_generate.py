...
class SRTGenerator:
    ...
    def _load_intent_profiles(self) -> Dict[str, Any]:
        ...
        try:
            ...
            log.warning(
                f"[srt-gen] Failed to load intent profiles: {e}, using defaults"
            )
            return self._get_default_intent_profiles()
    ...
    def _get_default_intent_profiles(self) -> Dict[str, Any]:
        ...
        return {
            "default": {"words_per_sec": [2.5, 3.5], "speech_ratio_default": 0.8},
            "narrative_history": {
    ...
    def generate_srt_for_acceptance(
        self,
        script_path: str,
        ...
        try:
            ...
            if asr_timings:
                log.info("[srt-gen] Using ASR timings for SRT generation")
                srt_content = self._generate_srt_from_timings(
                    script_content, asr_timings, "asr"
                )
                source = "asr"
            elif tts_timings:
                log.info("[srt-gen] Using TTS timings for SRT generation")
                srt_content = self._generate_srt_from_timings(
                    script_content, tts_timings, "tts"
                )
                source = "tts"
            else:
                log.info("[srt-gen] No timings found, using heuristic generation")
                srt_content = self._generate_srt_heuristic(
                    script_content, intent_type, target_duration_sec
                )
                source = "heuristic"
        ...
        try:
            ...
            if os.path.exists(asr_file):
                ...
                if "words" in asr_data and asr_data["words"]:
                    log.info(
                        f"[srt-gen] Found ASR timings with {len(asr_data['words'])} words"
                    )
                    return asr_data
        except Exception as e:
        ...
        try:
            ...
            if os.path.exists(tts_file):
                ...
                if "words" in tts_data and tts_data["words"]:
                    log.info(
                        f"[srt-gen] Found TTS timings with {len(tts_data['words'])} words"
                    )
                    return tts_data
        except Exception as e:
    ...
    def _generate_srt_from_timings(
        self, script_content: str, timings: Dict[str, Any], source: str
    ) -> str:
        ...
        if source == "asr":
            ...
            if len(word_timings) == len(words):
                ...
                for i, (word, timing) in enumerate(zip(words, word_timings)):
                    ...
                    if (
                        len(current_caption) >= max_words_per_caption
                        or i == len(words) - 1
                    ):
                ...
                log.warning(
                    f"[srt-gen] ASR word count mismatch: {len(word_timings)} vs {len(words)}"
                )
            ...
            if len(word_timings) == len(words):
                ...
                log.warning(
                    f"[srt-gen] TTS word count mismatch: {len(word_timings)} vs {len(words)}"
                )
                return self._generate_srt_heuristic(script_content, "default", None)

        return "\n".join(srt_lines)

    def _generate_srt_heuristic(
        self, script_content: str, intent_type: str, target_duration_sec: float
    ) -> str:
        ...
        # Get timing profile
        profile = self.intent_profiles.get(
            intent_type, self.intent_profiles.get("default", {})
        )
        ...
        if target_duration_sec:
            ...
            log.info(
                f"[srt-gen] Adjusted words per second to {words_per_sec:.2f} for target duration"
            )
    ...
    def _validate_generated_srt(
        self, srt_path: str, script_content: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "issues": issues,
                "avg_caption_duration": (
                    total_duration / total_captions if total_captions > 0 else 0
                ),
            }
    ...
    def _parse_timestamp_line(self, timestamp_line: str) -> Tuple[float, float]:
        ...
        try:
            ...
            log.warning(
                f"[srt-gen] Failed to parse timestamp: {timestamp_line}, error: {e}"
            )
...
def generate_srt_for_acceptance(
    ...
    return generator.generate_srt_for_acceptance(
        script_path, output_path, intent_type, target_duration_sec
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate SRT captions with fallback timing"
    )
    ...
    result = generate_srt_for_acceptance(
        args.script_path, args.output, args.intent, args.duration
    )
