...
def segment_text_for_captions(text: str, max_words_per_caption: int = 8) -> List[str]:
    ...
    for sentence in sentences:
        ...
        if (
            current_caption
            and len(current_caption.split()) + len(sentence.split())
...
def generate_synthetic_srt(
    ...
    # Calculate timing
    caption_timings = calculate_caption_timing(captions, audio_duration_sec, wpm, seed)
    ...
    try:
        ...
        log.info(
            f"Captions: {len(captions)}, Duration: {audio_duration_sec:.1f}s, WPM: {wpm}"
        )
    except Exception as e:
...
def main():
    ...
    parser.add_argument(
        "--wpm", type=int, default=160, help="Words per minute (default: 160)"
    )
    parser.add_argument(
        "--max-words", type=int, default=8, help="Max words per caption (default: 8)"
    )
