...
class MemoryMonitor:
    ...
    def _log_memory_status(self, memory_info: Dict[str, Any]):
        ...
        log.info(f"  System: {system['used_gb']:.2f}GB / {system['total_gb']:.2f}GB ({system['percent']:.1f}%)")
        log.info(f"  Swap: {system['swap_used_gb']:.2f}GB / {system['swap_total_gb']:.2f}GB ({system['swap_percent']:.1f}%)")
        
        if "error" not in ollama:
            log.info(f"  Ollama: {ollama['total_memory_gb']:.3f}GB ({len(ollama['processes'])} processes)")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Memory Monitor for 8GB M2 MacBook Air")
