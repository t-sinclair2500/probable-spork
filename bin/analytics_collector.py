...
class MetricsCollector:
    ...
    def parse_state_logs(self, hours: int = 24) -> Dict[str, Any]:
        ...
        steps = defaultdict(lambda: {"count": 0, "statuses": defaultdict(int), "durations": []})
        errors = []
        recent_activities = []
    ...
    def analyze_content_quality(self) -> Dict[str, Any]:
        """Analyze content quality metrics from recent blog posts."""
        metrics = {
            "posts_analyzed": 0,
            "avg_word_count": 0,
            "fact_check_issues": {"total": 0, "by_severity": {"error": 0, "warning": 0, "info": 0}},
            "validation_scores": [],
            "content_trends": {}
        }
        
        try:
            # Look for recent blog metadata
            ...
            if os.path.exists(meta_file):
                ...
                metrics["avg_word_count"] = validation.get("metrics", {}).get("word_count", 0)
                
                if fact_check:
                    fact_metrics = fact_check.get("metrics", {})
                    severity_counts = fact_metrics.get("severity_counts", {})
                    
                    metrics["fact_check_issues"]["total"] = fact_metrics.get("total_issues", 0)
            ...
            for folder in os.listdir(assets_dir):
                ...
                if os.path.exists(license_file):
                    ...
                    try:
                        ...
                        if quality_scores:
                            metrics["quality_metrics"]["avg_overall_score"] = sum(quality_scores) / len(quality_scores)
                            metrics["quality_metrics"]["assets_analyzed"] += len(quality_scores)
                        
                        if relevance_scores:
                            metrics["quality_metrics"]["avg_relevance_score"] = sum(relevance_scores) / len(relevance_scores)
                        
                        metrics["quality_metrics"]["total_quality_issues"] += total_issues
    ...
    def get_trend_data(self, hours: int = 24) -> Dict[str, Any]:
        ...
        trend_data = {"timestamps": [], "cpu": [], "memory": [], "disk": [], "pipeline_success": []}
        ...
        for days_back in range(2):  # Check last 2 days
            ...
            try:
                ...
                for entry in daily_data:
                    ...
                    trend_data["memory"].append(system.get("memory", {}).get("percent", 0))
                    ...
                    if performance:
                        success_rates = [perf.get("success_rate", 0) for perf in performance.values()]
