...
class SEOEnhancer:
    ...
    def find_featured_image(
        self, post_metadata: Dict[str, Any]
        ...
        if slug:
            ...
            if os.path.exists(assets_base):
                ...
                for folder in os.listdir(assets_base):
                    if slug in folder and os.path.isdir(
                        os.path.join(assets_base, folder)
                    ):
    ...
    def analyze_content_quality(
        self, content: str, post_metadata: Dict[str, Any]
    ) -> Dict[str, float]:
    ...
    def generate_seo_metadata(
        ...
        # Featured image
        img_url, img_alt, img_width, img_height = self.find_featured_image(
            post_metadata
        )
        ...
        if fact_check:
            ...
            if not issues:
                ...
                fact_check_score = max(
                    0, 100 - (error_count * 20) - (warning_count * 10)
                )
    ...
    def generate_html_head(self, seo_metadata: SEOMetadata) -> str:
        ...
        head_parts.append(
            f'<meta name="description" content="{seo_metadata.description}">'
        )
        ...
        head_parts.append(
            f'<meta property="og:description" content="{seo_metadata.description}">'
        )
        head_parts.append(f'<meta property="og:url" content="{seo_metadata.url}">')
        head_parts.append(
            f'<meta property="og:site_name" content="{self.config["site_name"]}">'
        )

        if seo_metadata.featured_image_url:
            head_parts.append(
                f'<meta property="og:image" content="{seo_metadata.featured_image_url}">'
            )
            head_parts.append(
                f'<meta property="og:image:alt" content="{seo_metadata.featured_image_alt}">'
            )
            head_parts.append(
                f'<meta property="og:image:width" content="{seo_metadata.featured_image_width}">'
            )
            head_parts.append(
                f'<meta property="og:image:height" content="{seo_metadata.featured_image_height}">'
            )

        # Article-specific Open Graph
        head_parts.append(
            f'<meta property="article:published_time" content="{seo_metadata.published_date}">'
        )
        head_parts.append(
            f'<meta property="article:modified_time" content="{seo_metadata.modified_date}">'
        )
        head_parts.append(
            f'<meta property="article:author" content="{seo_metadata.author_name}">'
        )
        ...
        head_parts.append(
            f'<meta name="twitter:card" content="{seo_metadata.twitter_card}">'
        )
        if seo_metadata.twitter_site:
            head_parts.append(
                f'<meta name="twitter:site" content="{seo_metadata.twitter_site}">'
            )
        if seo_metadata.twitter_creator:
            head_parts.append(
                f'<meta name="twitter:creator" content="{seo_metadata.twitter_creator}">'
            )
        head_parts.append(f'<meta name="twitter:title" content="{seo_metadata.title}">')
        head_parts.append(
            f'<meta name="twitter:description" content="{seo_metadata.description}">'
        )

        if seo_metadata.featured_image_url:
            head_parts.append(
                f'<meta name="twitter:image" content="{seo_metadata.featured_image_url}">'
            )
            head_parts.append(
                f'<meta name="twitter:image:alt" content="{seo_metadata.featured_image_alt}">'
            )

        # Additional meta tags for reading time
        head_parts.append(
            f'<meta name="reading-time" content="{seo_metadata.reading_time_minutes}">'
        )
        head_parts.append(
            f'<meta name="word-count" content="{seo_metadata.word_count}">'
        )
    ...
    def generate_schema_markup(self, seo_metadata: SEOMetadata) -> str:
        ...
        article_schema = {
            ...
            "mainEntityOfPage": {"@type": "WebPage", "@id": seo_metadata.url},
        ...
        if seo_metadata.fact_check_score is not None:
            ...
            article_schema["creditText"] = (
                f"Fact-check score: {seo_metadata.fact_check_score:.1f}/100"
            )
