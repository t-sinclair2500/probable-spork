...
Examples:
  ...
  python bin/music_manager.py test --script scripts/test.txt --tone conversational --duration 30
    ...
    import_parser.add_argument('--source', required=True, help='Source directory containing music files')
    import_parser.add_argument('--license', default='unknown', help='License type for imported tracks')
    import_parser.add_argument('--source-name', default='local', help='Source name for imported tracks')
    
    # Stats command
    stats_parser = subparsers.add_parser('stats', help='View music library statistics')
    
    # Validate command
    validate_parser = subparsers.add_parser('validate', help='Validate music library integrity')
    
    # Test command
    test_parser = subparsers.add_parser('test', help='Test music selection system')
    test_parser.add_argument('--script', required=True, help='Path to script file for content analysis')
    test_parser.add_argument('--tone', default='conversational', help='Video tone')
    test_parser.add_argument('--duration', type=float, default=30.0, help='Video duration in seconds')
    test_parser.add_argument('--pacing-wpm', type=int, default=165, help='Words per minute pacing')
    ...
    # Setup command
    setup_parser = subparsers.add_parser('setup', help='Setup music library with sample tracks')
...
def handle_import(manager: MusicIntegrationManager, args: argparse.Namespace) -> int:
    ...
    imported_count = manager.create_music_library_from_directory(source_dir, license_info)
...
def handle_setup(manager: MusicIntegrationManager) -> int:
        ...
        if not file_path.exists():
            ...
            try:
                ...
                cmd = [
                    ...
                    '-t', '1', '-c:a', 'libmp3lame', '-b:a', '128k', str(file_path)
                ]
    ...
    print("  2. Run: python bin/music_manager.py import --source assets/music --license 'your-license'")
