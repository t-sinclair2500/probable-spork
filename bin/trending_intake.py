...
from bin.core import BASE, get_logger, load_env, log_state, single_lock
...
class TrendingIntake:
    ...
    def run(self, mode: str, providers: List[str], limit: int) -> List[Dict]:
        """Main run method"""
        log.info(
            f"Starting trending intake: mode={mode}, providers={providers}, limit={limit}"
        )
...
def main():
    ...
    parser.add_argument(
        "--limit", type=int, default=20, help="Maximum number of topics to return"
    )
    parser.add_argument(
        "--slug", help="Topic slug for logging and artifact organization"
    )
    ...
    try:
        ...
        log_state(
            "trending_intake",
            "OK",
            f"mode={args.mode}, topics={len(topics)}{slug_info}",
        )
