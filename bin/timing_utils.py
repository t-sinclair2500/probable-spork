...
def distribute_duration_uniform(
    ...
    log.info(f"Uniformly distributed {target_ms}ms across {num_scenes} scenes: {clamped}")
    return clamped
...
def align_scenes_to_vo(
    ...
    if len(scenes) != len(vo_scenes):
        log.warning(f"Scene count mismatch: {len(scenes)} scenes vs {len(vo_scenes)} VO cues")
        return []
...
def compute_scene_durations(
    ...
    if any(beat.get('duration_ms') for beat in beats):
        durations = []
        for beat in beats:
            duration = beat.get('duration_ms', timing_config.get('default_scene_ms', 5000))
            durations.append(duration)
    ...
    if strategy == 'weighted':
        durations = distribute_duration_by_words(beats, target_ms, min_scene_ms, max_scene_ms)
