...
from bin.cutout.qa_gates import run_all as run_qa_gates
...
def truncate_text(text: str, max_words: int = MAX_WORDS_PER_CARD) -> str:
    ...
    log.info(
        f"Truncated text from {len(words)} to {max_words} words: {truncated[:50]}..."
    )
    return truncated


def create_text_element(
    content: str, element_id: str, x: float, y: float, font_size: str = "body"
) -> Element:
...
def apply_legibility_defaults(
    scene: Scene, brand_style: BrandStyle, log: logging.Logger
) -> Scene:
    ...
    try:
        ...
        log.warning(
            f"[legibility-defaults] Failed to load legibility config: {e}, using defaults"
        )
    ...
    if not scene.bg:
        ...
        log.info(
            f"[legibility-defaults] Applied default background 'gradient1' to scene {scene.id}"
        )
    ...
    for element in scene.elements:
        if element.type == "text" and not element.style.get("color"):
            ...
            if bg_color:
                ...
                log.info(
                    f"[legibility-defaults] Applied default text color {text_color} to element {element.id} in scene {scene.id}"
                )

    return scene


def apply_legibility_defaults_to_storyboard(
    storyboard: SceneScript, brand_style: BrandStyle, log: logging.Logger
) -> SceneScript:
    ...
    log.info(
        f"[legibility-defaults] Applied legibility defaults to {len(storyboard.scenes)} scenes"
    )
    return storyboard
...
def map_background_to_color(bg_id: str, brand_style: BrandStyle) -> str:
    ...
    try:
        ...
        cfg = load_config()
        background_mapping = getattr(cfg, "legibility", {}).get(
            "background_mapping", {}
        )
        ...
        log.warning(
            f"[legibility-defaults] Failed to load background mapping config: {e}"
        )
...
def get_contrasting_text_color(bg_color: str, brand_style: BrandStyle) -> str:
    ...
    try:
        ...
        cfg = load_config()
        text_color_fallback = getattr(cfg, "legibility", {}).get(
            "text_color_fallback", {}
        )

        # Use config fallback colors if available
        light_fallback = text_color_fallback.get(
            "light_background", brand_style.colors.get("text_primary", "#111827")
        )
        dark_fallback = text_color_fallback.get(
            "dark_background", brand_style.colors.get("white", "#ffffff")
        )
    except Exception as e:
        log.warning(
            f"[legibility-defaults] Failed to load text color fallback config: {e}"
        )
...
def create_scene_from_beat(
    beat: Dict, scene_id: str, duration_ms: int, brand_style: BrandStyle
) -> Scene:
    ...
    main_element.keyframes = [Keyframe(t=0, opacity=0.0), Keyframe(t=200, opacity=1.0)]
...
def map_beats_to_scenes(
    ...
    durations, strategy, rationale = compute_scene_durations(
        beats, brief, timing_config, slug
    )
...
def create_storyboard(
    slug: str, beats: List[Dict], brief: Dict, brand_style: BrandStyle
) -> SceneScript:
    ...
    target_sec = (
        target_min if target_min == target_max else (target_min + target_max) / 2
    )
    ...
    deviation_pct = (
        abs(total_duration_ms - target_ms) / target_ms * 100 if target_ms > 0 else 0
    )
    timing_compliant = deviation_pct <= timing_tolerance

    log.info(
        f"[duration-policy] Created {total_scenes} scenes, total duration: {total_duration_ms}ms"
    )
    log.info(
        f"[duration-policy] Target: {target_ms}ms, deviation: {deviation_pct:.1f}%, compliant: {timing_compliant}"
    )
    ...
    if truncation_count > 0:
        log.info(
            f"Applied {truncation_count} text truncations to respect {MAX_WORDS_PER_CARD} word limit"
        )
    ...
    try:
        ...
        layout_strategy = (
            procedural_cfg.layout.get("strategy", "auto")
            if procedural_cfg.layout
            else "auto"
        )
        if layout_strategy != "manual":
            ...
            for scene in scenes:
                ...
                needs_layout = any(
                    ("x" not in e or "y" not in e) for e in scene.elements
                )
                if needs_layout:
                    ...
                    cfg_dict = {
                        "procedural": {
                            ...
                            "placement": {
                                "min_spacing_px": (
                                    getattr(
                                        procedural_cfg.placement, "min_spacing_px", 64
                                    )
                                    if procedural_cfg.placement
                                    else 64
                                ),
                                "safe_margin_px": (
                                    getattr(
                                        procedural_cfg.placement, "safe_margin_px", 40
                                    )
                                    if procedural_cfg.placement
                                    else 40
                                ),
                            ...
                            "layout": {
                                "strategy": (
                                    getattr(procedural_cfg.layout, "strategy", "auto")
                                    if procedural_cfg.layout
                                    else "auto"
                                ),
                                "prefer_thirds": (
                                    getattr(
                                        procedural_cfg.layout, "prefer_thirds", True
                                    )
                                    if procedural_cfg.layout
                                    else True
                                ),
                                "max_attempts": (
                                    getattr(procedural_cfg.layout, "max_attempts", 200)
                                    if procedural_cfg.layout
                                    else 200
                                ),
                            },
                    ...
                    scene_dict = auto_layout_scene(
                        scene_dict, cfg_dict, seed + hash(scene.id) % 10000
                    )
...
def update_existing_scenescript_legibility(
    slug: str, brand_style: BrandStyle, log: logging.Logger
) -> bool:
    ...
    if not scenescript_path.exists():
        log.info(
            f"[legibility-defaults] No existing scenescript found at {scenescript_path}"
        )
        return False
    ...
    try:
        ...
        for scene in storyboard.scenes:
            ...
            for element in scene.elements:
                if element.type == "text" and (
                    not element.style or not element.style.get("color")
                ):
                    needs_update = True
                    break
            if needs_update:
        ...
        if not needs_update:
            log.info(
                f"[legibility-defaults] Existing scenescript {slug} already has legibility defaults"
            )
            return True

        # Apply legibility defaults
        log.info(
            f"[legibility-defaults] Updating existing scenescript {slug} with legibility defaults"
        )
        storyboard = apply_legibility_defaults_to_storyboard(
            storyboard, brand_style, log
        )

        # Save updated scenescript
        save_scene_script(storyboard, scenescript_path)
        log.info(
            f"[legibility-defaults] Updated and saved scenescript {slug} with legibility defaults"
        )
        ...
        log.error(
            f"[legibility-defaults] Failed to update existing scenescript {slug}: {e}"
        )
        return False
...
def main(brief=None, models_config=None):
    ...
    parser = argparse.ArgumentParser(
        description="Convert grounded beats to SceneScript"
    )
    ...
    qa_results = []
    palette = (
        brand_style.colors.values()
        if hasattr(brand_style.colors, "values")
        else list(brand_style.colors.values())
    )

    for i, scene in enumerate(storyboard.scenes):
        progress = ((i + 1) / len(storyboard.scenes)) * 100
        print(
            f"   🎯 Scene {i+1}/{len(storyboard.scenes)} ({progress:.1f}%): {scene.id}"
        )
    ...
    log.info(
        f"QA Gates completed: {len(qa_results)} scenes checked, {total_qa_issues} with issues"
    )
    ...
    try:
        updated_storyboard, coverage_results = run_asset_loop(
            args.slug, storyboard, brand_style, seed=42
        )
        ...
        if coverage_results["is_fully_covered"]:
            ...
            print(
                f"   ⚠️  Asset loop completed with {coverage_results['coverage_pct']:.1f}% coverage"
            )
            log.warning(
                f"Asset loop completed with {coverage_results['coverage_pct']:.1f}% coverage"
            )
            # Continue with partial coverage - assets will be generated during rendering

        print(
            f"   📊 Coverage: {coverage_results['covered_requirements']}/{coverage_results['total_requirements']} ({coverage_results['coverage_pct']:.1f}%)"
        )
        log.info(
            f"Asset coverage: {coverage_results['covered_requirements']}/{coverage_results['total_requirements']} ({coverage_results['coverage_pct']:.1f}%)"
        )
        ...
        log.warning(
            "Continuing with original storyboard - assets will be generated during rendering"
        )
    ...
    print(
        f"⏱️  Total Duration: {sum(s.duration_ms for s in storyboard.scenes)/1000:.1f}s"
    )
