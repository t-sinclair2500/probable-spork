...
def generate_outline(
    topic: str, target_len_sec: int = 60, brief: Dict = None, models_config: Dict = None
) -> Dict:
    """
    ...
    try:
        ...
        try:
            ...
            log.info(
                f"[outline] Loaded intent template: {intent} (evidence_load: {intent_metadata['evidence_load']}, CTA: {intent_metadata['cta_policy']})"
            )
        except Exception as e:
            log.warning(
                f"[outline] Failed to load intent template '{intent}': {e}, using fallback"
            )
            intent_template = None
            intent_metadata = None
        ...
        if intent_template:
            try:
                ...
                log.info(
                    f"[outline] Citation requirements: {citation_reqs['beats_needing_citations']}/{citation_reqs['total_beats']} beats need citations ({citation_reqs['coverage_percentage']:.1f}% coverage)"
                )
            except Exception as e:
        ...
        evidence_load = (
            intent_metadata["evidence_load"] if intent_metadata else "medium"
        )
        ...
        system_prompt = template.format(
            ...
            intent=intent,
            cta_policy=(
                intent_metadata.get("cta_policy", "optional")
                if intent_metadata
                else "optional"
            ),
        )
        ...
        if intent_template:
            ...
            log.info(
                f"[outline] Applied intent template: {len(data['sections'])} sections, {sum(1 for s in data['sections'] if s.get('needs_citations', False))} beats need citations"
            )
        ...
        if not data.get("sections"):
            if intent_template:
                ...
                log.info(
                    f"[outline] Created fallback outline using intent template: {len(data['sections'])} sections"
                )
            else:
        ...
        citations_count = sum(
            1 for s in data.get("sections", []) if s.get("needs_citations", False)
        )

        log_state(
            "llm_outline",
            "OK",
            f"{os.path.basename(topic)};intent={intent};beats={beats_count};citations={citations_count}",
        )

        # Print summary
        print(f"Generated outline for: {topic}")
        print(
            f"Intent: {intent}, Beats: {beats_count}, Citations needed: {citations_count}"
        )
...
def validate_and_enhance_outline(data: Dict, target_len_sec: int) -> Dict:
    ...
    if not isinstance(data, dict):
        return create_fallback_outline(
            data.get("topic", "Unknown"), target_len_sec, "narrative_history"
        )
    ...
    if total_duration > 0:
        ...
        if abs(total_duration - target_ms) > 5000:  # More than 5 seconds off
            ...
            for section in data["sections"]:
                section["target_duration_ms"] = int(
                    section["target_duration_ms"] * scale_factor
                )
...
def main(brief=None, models_config=None):
    ...
    parser.add_argument(
        "--slug", required=True, help="Topic slug for outline generation"
    )
    ...
    log.info(
        f"[outline] Starting outline generation for slug: {args.slug}, topic: {topic}, duration: {args.duration}s"
    )
