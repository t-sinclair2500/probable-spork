...
class AssetLibrarian:
    ...
    def _load_manifest(self) -> Dict[str, Any]:
        ...
        try:
            with open(self.manifest_path, "r") as f:
                ...
                log.info(
                    f"[librarian] Loaded manifest with {len(manifest.get('assets', {}))} assets"
                )
                return manifest
    ...
    def _load_modules_config(self) -> Dict[str, Any]:
        ...
        return {
            "reuse_threshold": 0.7,
            "generation_caps": {"max_new_assets": 10, "max_per_section": 3},
        }
    ...
    def _find_matching_assets(
        ...
        for asset_hash, asset in assets.items():
            ...
            if (
                category.lower() not in asset_path.lower()
                and category.lower() not in str(asset.get("tags", [])).lower()
            ):
                continue
            ...
            if size_constraints:
                ...
                if asset_w and asset_h:
                    ...
                    target_aspect = size_constraints.get("w", 1) / size_constraints.get(
                        "h", 1
                    )
    ...
    def _select_variant(
        self, asset: Dict[str, Any], requested_variants: Optional[List[str]] = None
    ) -> str:
    ...
    def _calculate_scale(
        self, asset: Dict[str, Any], size_constraints: Optional[Dict[str, float]] = None
    ) -> float:
    ...
    def resolve_assets(
        ...
        reuse_ratio = (
            len(resolved) / total_placeholders if total_placeholders > 0 else 0.0
        )
        ...
        log.info(
            f"[librarian] Resolved {len(resolved)}/{total_placeholders} assets (reuse ratio: {reuse_ratio:.2%})"
        )
    ...
    def _resolve_element(
        self, element: Dict[str, Any], scene_id: str
    ) -> Optional[Dict[str, Any]]:
        ...
        log.debug(
            f"[librarian] Resolving {element_type} {element_id} for scene {scene_id}"
        )
        ...
        for style_hint in style_hints:
            ...
            if matches:
                ...
                log.info(
                    f"[librarian] Reused {element_type} {asset['path']} for {element_id}"
                )
...
def main():
    ...
    parser.add_argument(
        "--seed", type=int, help="Random seed for deterministic asset selection"
    )
    ...
    try:
        ...
        print(
            f"  Resolved: {asset_plan['resolved_count']}/{asset_plan['total_placeholders']}"
        )
        ...
        if asset_plan["gaps"]:
            ...
            for gap in asset_plan["gaps"][:5]:  # Show first 5
                print(
                    f"  {gap['element_id']}: {gap['spec']['category']} - {gap['spec']['style']}"
                )
