...
def main(brief=None, models_config=None):
    ...
    # Build prompt
    with open(
        os.path.join(BASE, "prompts", "cluster_topics.txt"), "r", encoding="utf-8"
    ) as f:
        template = f.read()
    ...
    try:
        ...
        if brief and brief.get("keywords_include"):
            primary_keyword = (
                brief["keywords_include"][0] if brief["keywords_include"] else "topic"
            )
            topics = [
                {
    ...
    if brief and brief.get("keywords_exclude"):
        ...
        for topic in topics:
            ...
            if not any(exclude_term in topic_text for exclude_term in exclude_terms):
                ...
                log.info(
                    f"Filtered out topic '{topic.get('topic')}' due to excluded keywords"
                )
