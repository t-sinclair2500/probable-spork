...
def map_script_to_beats(
    script_lines: List[str], grounded_beats: List[Dict]
) -> Dict[int, Dict]:
...
def analyze_factual_claims(
    script_lines: List[str],
    grounded_beats: List[Dict],
    ...
    """
    log.info(
        f"[fact-guard] Starting factual claims analysis with strictness: {strictness}"
    )
    ...
    current_level = strictness_levels.get(
        strictness, strictness_levels.get("balanced", {})
    )
    ...
    for line_num, line in enumerate(script_lines, 1):
        ...
        for claim in line_claims:
            ...
            if action == "rewrite":
                claim["suggested_text"] = generate_cautious_text(
                    line, claim["claim_type"]
                )
...
def analyze_line_for_claims(
    line: str, claim_policies: Dict, has_citations: bool
) -> List[Dict]:
            ...
            claims.append(
                {
                    ...
                    "requires_citation": claim_policies.get("proper_nouns", {}).get(
                        "requires_citation", True
                    ),
                    "rationale": claim_policies.get("proper_nouns", {}).get(
                        "rationale",
                        "Proper nouns often represent specific facts requiring verification",
                    ),
                }
            )
            ...
            claims.append(
                {
                    "claim_type": "dates",
                    "claim_text": match.group(),
                    "requires_citation": claim_policies.get("dates", {}).get(
                        "requires_citation", True
                    ),
                    "rationale": claim_policies.get("dates", {}).get(
                        "rationale",
                        "Specific dates are factual claims needing source verification",
                    ),
                }
            )
            ...
            claims.append(
                {
                    ...
                    "claim_text": match.group(),
                    "requires_citation": claim_policies.get("superlatives", {}).get(
                        "requires_citation", True
                    ),
                    "rationale": claim_policies.get("superlatives", {}).get(
                        "rationale",
                        "Superlatives (first, most, best) are factual claims needing evidence",
                    ),
                }
            )
            ...
            claims.append(
                {
                    ...
                    "claim_text": match.group(),
                    "requires_citation": claim_policies.get("statistics", {}).get(
                        "requires_citation", True
                    ),
                    "rationale": claim_policies.get("statistics", {}).get(
                        "rationale",
                        "Statistics without sources are unreliable and should be removed",
                    ),
                }
            )
            ...
            claims.append(
                {
                    ...
                    "claim_text": match.group(),
                    "requires_citation": claim_policies.get("expert_opinions", {}).get(
                        "requires_citation", True
                    ),
                    "rationale": claim_policies.get("expert_opinions", {}).get(
                        "rationale",
                        "Expert opinions need attribution to maintain credibility",
                    ),
                }
            )
                ...
                claims.append(
                    {
                        ...
                        "claim_text": line,
                        "requires_citation": claim_policies.get(
                            "general_statements", {}
                        ).get("requires_citation", False),
                        "rationale": claim_policies.get("general_statements", {}).get(
                            "rationale",
                            "General observations don't require specific citations",
                        ),
                    }
                )
                break
...
def determine_claim_action(
    claim: Dict, strictness_level: Dict, claim_policies: Dict
) -> str:
...
def generate_cautious_text(original_text: str, claim_type: str) -> str:
    ...
    if claim_type == "proper_nouns":
        ...
        return (
            ...
            if "million" in original_text.lower() or "billion" in original_text.lower()
            else "some"
        )
    elif claim_type == "expert_opinions":
        # Make opinions more general
        return "some people believe" + original_text.replace("expert says", "").replace(
            "expert said", ""
        )
    else:
...
def apply_fact_guard_changes(
    script_lines: List[str], analysis_results: Dict, strictness: str = "balanced"
) -> Tuple[List[str], Dict]:
    """
    ...
    changes_applied = {"kept": [], "removed": [], "rewritten": [], "flagged": []}

    # Sort claims by line number (descending) to avoid line number shifts
    claims = sorted(
        analysis_results.get("claims", []), key=lambda x: x.get("line", 0), reverse=True
    )
    ...
    for claim in claims:
        ...
        if action == "remove":
            ...
            if line_idx < len(cleaned_lines):
                ...
                log.info(
                    f"[fact-guard] Removed line {line_num}: {removed_line[:50]}..."
                )
            ...
            if line_idx < len(cleaned_lines):
                ...
                log.info(
                    f"[fact-guard] Rewrote line {line_num}: {old_text[:50]}... -> {suggested_text[:50]}..."
                )
    ...
    log.info(
        f"[fact-guard] Changes applied: {len(changes_applied['removed'])} removed, {len(changes_applied['rewritten'])} rewritten, {len(changes_applied['flagged'])} flagged"
    )
...
def run_fact_guard_analysis(slug: str, strictness: str = "balanced") -> Dict:
    ...
    try:
        ...
        analysis_results = analyze_factual_claims(
            script_lines, grounded_beats, references, strictness
        )

        # Apply changes if needed
        cleaned_script, changes_applied = apply_fact_guard_changes(
            script_lines, analysis_results, strictness
        )
        ...
        fact_guard_report = {
            ...
            "metadata": {
                ...
                "config_used": analysis_results.get("metadata", {}).get(
                    "config_used", {}
                ),
            },
        }
...
def main(slug: str = None, strictness: str = "balanced"):
    ...
    parser = argparse.ArgumentParser(
        description="Run fact-guard analysis on script content"
    )
    parser.add_argument(
        "--slug", required=True, help="Topic slug for fact-guard analysis"
    )
    parser.add_argument(
        "--strictness",
    ...
    log.info(
        f"[fact-guard] Starting fact-guard analysis for slug: {slug}, strictness: {strictness}"
    )
