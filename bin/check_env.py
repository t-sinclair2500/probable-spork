...
def check_ffmpeg_installation():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            ffmpeg_info['ffmpeg'] = {'available': False, 'error': 'version_check_failed'}
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            ffmpeg_info['ffprobe'] = {'available': False, 'error': 'version_check_failed'}
    except FileNotFoundError:
    ...
    if ffmpeg_info.get('ffmpeg', {}).get('available') and ffmpeg_info.get('ffprobe', {}).get('available'):
        log.info("FFmpeg and FFprobe are both available and working")
    else:
        issues.append("FFmpeg/FFprobe installation incomplete - audio validation will fail")
...
def main():
    ...
    try:
        render_path = os.path.join(os.path.dirname(__file__), "..", "conf", "render.yaml")
        if os.path.exists(render_path):
            ...
            if not acceptance:
                ...
                if "audio_validation_required" not in acceptance:
                    issues.append("acceptance.audio_validation_required not set in render.yaml")
                if "caption_validation_required" not in acceptance:
                    issues.append("acceptance.caption_validation_required not set in render.yaml")
                if "legibility_validation_required" not in acceptance:
                    issues.append("acceptance.legibility_validation_required not set in render.yaml")
            ...
            issues.append("render.yaml not found - audio validation and acceptance pipeline may fail")
