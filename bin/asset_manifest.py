...
class AssetManifest:
        ...
        if self.manifest_path.exists():
            try:
                with open(self.manifest_path, "r") as f:
                    data = json.load(f)
                    log.info(
                        f"Loaded existing manifest with {len(data.get('assets', {}))} assets"
                    )
                    return data
    ...
    def _extract_svg_metadata(self, svg_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            palette_ok, delta_e_violations = self._validate_palette_compliance_delta_e(
                colors
            )
    ...
    def _validate_palette_compliance_delta_e(
    ...
    def _generate_thumbnail_pillow(self, svg_path: Path, thumbnail_path: Path) -> bool:
        ...
        try:
            ...
            log.warning(
                f"Pillow/Cairo thumbnail generation failed for {svg_path.name}: {e}"
            )
            return False
        ...
        try:
            ...
            if PILLOW_AVAILABLE or CAIRO_AVAILABLE:
                if self._generate_thumbnail_pillow(svg_path, thumbnail_path):
                    log.debug(
                        f"Generated thumbnail with Pillow/Cairo for {svg_path.name}"
                    )
                    return True
            ...
            if result.returncode == 0:
                ...
                log.warning(
                    f"Failed to generate thumbnail for {svg_path.name}: {result.stderr}"
                )
                return False
    ...
    def rebuild_manifest(self, filter_palette_only: bool = False) -> Dict[str, Any]:
        ...
        for svg_path in svg_files:
            ...
            is_compliant, color_violations = self._validate_palette_compliance_delta_e(
                metadata["palette"]
            )
            ...
            if not is_compliant:
                ...
                log.warning(
                    f"Palette violations in {svg_path.name}: {len(metadata['delta_e_violations'])} violations"
                )
        ...
        log.info(
            f"[manifest] Manifest rebuilt: {len(new_assets)} assets, {len(violations)} violations"
        )
    ...
    def get_manifest_summary(self) -> Dict[str, Any]:
        ...
        # Count by source
        brand_count = sum(
            1 for a in assets.values() if a["provenance"]["source"] == "brand"
        )
        generated_count = sum(
            1 for a in assets.values() if a["provenance"]["source"] == "generated"
        )
...
def main():
    ...
    parser.add_argument(
        "--rebuild", action="store_true", help="Rebuild the complete manifest"
    )
    parser.add_argument(
        "--filter", choices=["palette-only"], help="Filter mode for violations"
    )
        ...
        if args.filter == "palette-only":
            ...
            if violations:
                ...
                for violation in violations[:5]:  # Show first 5
                    print(
                        f"  {violation['asset']}: {len(violation['violations'])} violations"
                    )
                if len(violations) > 5:
    ...
    if args.summary or not args.rebuild:
        ...
        print(
            f"  Tag distribution: {dict(list(summary['tag_distribution'].items())[:5])}"
        )
