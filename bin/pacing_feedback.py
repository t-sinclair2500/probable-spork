...
def suggest_adjustments(kpi: Dict, bands: Dict, scenes: List[Dict], cfg: Dict) -> List[Dict]:
    """
    ...
    if total_adjustment > max_total_ms:
        log.warning(f"[pacing-feedback] Total adjustment {total_adjustment}ms exceeds limit {max_total_ms}ms, scaling down")
        adjustments = _scale_adjustments(adjustments, max_total_ms)
    
    log.info(f"[pacing-feedback] Suggested {len([a for a in adjustments if a['delta_ms'] != 0])} adjustments, "
             f"total delta: {total_adjustment}ms")
    
    return adjustments


def apply_adjustments(scenes: List[Dict], adjustments: List[Dict], cfg: Dict) -> List[Dict]:
    """
    ...
    for i, scene in enumerate(scenes):
        ...
        if align_to_vo and _has_vo_timing(scene):
            log.info(f"[pacing-feedback] Scene {scene_id} has VO timing, adjusting internal timing only")
...
def _scale_adjustments(adjustments: List[Dict], max_total_ms: int) -> List[Dict]:
    ...
    for adj in adjustments:
        ...
        scaled_adjustments.append({
            "id": adj["id"],
            "delta_ms": scaled_delta
        })
...
def save_adjustments_report(slug: str, adjustments: List[Dict], report_dir: str = None) -> str:
...
def update_pacing_report_with_adjustments(slug: str, adjustments: List[Dict], report_dir: str = None) -> str:
    ...
    try:
        ...
        report_data["metadata"]["adjustments_count"] = len([a for a in adjustments if a["delta_ms"] != 0])
...
def run_feedback_adjustment(slug: str, kpi_data: Dict, scenes: List[Dict], cfg: Dict) -> Tuple[List[Dict], Dict]:
    """
    ...
    log.info(f"[pacing-feedback] Feedback adjustment complete: {summary['adjusted_scenes']} scenes adjusted")
    
    return adjusted_scenes, summary


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Apply pacing feedback adjustments to scene durations")
    parser.add_argument("--slug", required=True, help="Content slug identifier")
    parser.add_argument("--apply", action="store_true", help="Apply adjustments and update metadata")
    parser.add_argument("--dry-run", action="store_true", help="Show proposed adjustments without applying")
    ...
    if args.dry_run:
        ...
        try:
            script_path = os.path.join(os.path.dirname(__file__), '..', 'scripts', f'{slug}.txt')
            srt_path = os.path.join(os.path.dirname(__file__), '..', 'voiceovers', f'{slug}.srt')
        ...
        adjustments = suggest_adjustments(pacing_data, pacing_data.get('comparison', {}).get('profile_used', {}), scenes, modules_cfg)
        ...
        if not any(adj["delta_ms"] != 0 for adj in adjustments):
            ...
            for adj in adjustments:
                if adj["delta_ms"] != 0:
                    scene_id = adj["id"]
                    current_duration = next((s.get('duration_ms', 0) for s in scenes if s.get('id') == scene_id), 0)
                    new_duration = current_duration + adj["delta_ms"]
                    print(f"  {scene_id}: {current_duration}ms → {new_duration}ms (Δ{adj['delta_ms']:+d}ms)")
        ...
        adjusted_scenes, summary = run_feedback_adjustment(slug, pacing_data, scenes, modules_cfg)
        
        if summary.get('adjusted'):
            print(f"✅ Adjustments applied successfully!")
            ...
            try:
                ...
                updated_kpi = compute_pacing(script_text, adjusted_scenes, video_ms, srt_path)
