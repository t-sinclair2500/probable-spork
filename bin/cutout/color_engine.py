...
from bin.utils.palette import load_palette as load_palette_adapter
...
def assert_legible_text(
    text_color: str, bg_color: str, large_text: bool = False
) -> None:
    """
...
def get_scene_color_policy() -> Dict[str, int]:
    ...
    try:
        config = load_config()
        return {"max_colors_per_scene": config.procedural.max_colors_per_scene}
...
def validate_scene_colors(scene_colors: List[str], scene_name: str = "unknown") -> bool:
    ...
    if len(scene_colors) > max_colors:
        log.error(
            f"Scene '{scene_name}' uses {len(scene_colors)} colors, max allowed: {max_colors}"
        )
        return False
