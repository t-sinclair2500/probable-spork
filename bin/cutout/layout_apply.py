...
from .sdk import SAFE_MARGINS_PX, VIDEO_W, VIDEO_H
from .layout_engine import poisson_points, pack_text_blocks, apply_constraints, thirds_grid
...
def auto_layout_scene(scene: dict, cfg: dict, seed: int) -> dict:
    ...
    text_elems = [e for e in scene["elements"] if e["type"] in ("text", "list_step", "lower_third")]
    prop_elems = [e for e in scene["elements"] if e["type"] in ("prop", "character", "shape", "counter")]
    
    log.debug(f"[layout] Scene {scene.get('id', 'unknown')}: {len(text_elems)} text, {len(prop_elems)} props")
            ...
            for e in text_elems:
                if "x" not in e or "y" not in e:
                    ...
                    if pid in by_id:
                        p = by_id[pid]
                        e["x"], e["y"] = p["x"], p["y"]
                        log.debug(f"[layout] id={e.get('id')} type={e['type']} pos=({e['x']},{e['y']}) method=text-pack")
        except Exception as e:
            log.warning(f"[layout] Text packing failed: {e}, falling back to center placement")
            for e in text_elems:
                if "x" not in e or "y" not in e:
                    e["x"], e["y"] = W // 2, H // 2
    ...
    if unpos:
        try:
            min_spacing = cfg.get("procedural", {}).get("placement", {}).get("min_spacing_px", 64)
            r = int(min_spacing)
            ...
            for e, (x, y) in zip(unpos, pts[:len(unpos)]):
                e["x"], e["y"] = int(x), int(y)
                log.debug(f"[layout] id={e.get('id')} type={e['type']} pos=({e['x']},{e['y']}) method=poisson")
        except Exception as e:
            log.warning(f"[layout] Poisson placement failed: {e}, falling back to grid placement")
...
def check_scene_layout_validity(scene: dict) -> bool:
    ...
    for e in scene.get("elements", []):
        if "x" in e and "y" in e:
            ...
            # Check safe margins
            if (x < SAFE_MARGINS_PX or 
                y < SAFE_MARGINS_PX or 
                x + w > VIDEO_W - SAFE_MARGINS_PX or 
                y + h > VIDEO_H - SAFE_MARGINS_PX):
                log.warning(f"[layout] Element {e.get('id')} outside safe margins: ({x},{y}) {w}x{h}")
                return False
