...
from .sdk import Element, Scene, Keyframe
...
class MicroAnimationGenerator:
    ...
    def _has_nearby_text(self, element: Element, scene: Scene, safe_distance: int = 80) -> bool:
        """
    ...
    def _calculate_animation_parameters(self, element: Element, scene_duration_ms: int) -> Dict[str, Any]:
        """
        ...
        keyframes = self._generate_keyframes(
            anim_type, scene_duration_ms, max_movement
        )
    ...
    def _generate_keyframes(self, anim_type: str, scene_duration_ms: int, max_movement: float) -> List[Keyframe]:
        """
        ...
        if anim_type == 'morph':
            ...
            keyframes = [
                ...
                Keyframe(t=scene_duration_ms // 4, x=max_movement * 0.5, y=max_movement * 0.3),
                Keyframe(t=3 * scene_duration_ms // 4, x=-max_movement * 0.3, y=max_movement * 0.5),
    ...
    def generate_scene_animations(self, scene: Scene) -> Dict[str, Any]:
        ...
        log.info(f"[micro-anim] Generating animations for scene {scene.id} with {len(scene.elements)} elements")
        
        # Find eligible elements
        eligible_elements = [
            elem for elem in scene.elements 
            if self._is_eligible_for_animation(elem, scene)
        ]
        
        # Limit to max percentage
        max_animated = max(1, int(len(scene.elements) * self.max_elements_percent / 100))
        elements_to_animate = eligible_elements[:max_animated]
        
        log.info(f"[micro-anim] {len(elements_to_animate)}/{len(scene.elements)} elements eligible for animation")
        
        # Generate animations for each element
        animations = []
        for element in elements_to_animate:
            try:
                anim_params = self._calculate_animation_parameters(element, scene.duration_ms)
                anim_params['keyframes'] = self._apply_ease_in_out(anim_params['keyframes'])
                ...
                log.info(f"[micro-anim] Element {element.id} ({element.type}): {anim_params['type']} "
        ...
        log.info(f"[micro-anim] Scene {scene.id}: {len(animations)} elements animated "
    ...
    def get_animation_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'animation_percentage': (total_animated / total_elements * 100) if total_elements > 0 else 0,
            'seed': self.seed,
...
def create_micro_animation_generator(config: Dict[str, Any], seed: Optional[int] = None) -> MicroAnimationGenerator:
    """
