...
def apply_texture_to_image(image_path: str, texture_config: Dict, brand_palette: Optional[list] = None) -> str:
    ...
    try:
        ...
        textured_path = str(base_path.parent / f"{base_path.stem}_textured{base_path.suffix}")
...
def apply_texture_with_qa_loop(
    ...
    log.info(f"[texture-qa] Starting texture application with QA loop (max retries: {max_retries})")
    
    while attempts <= max_retries:
        attempts += 1
        log.info(f"[texture-qa] Attempt {attempts}: applying textures with config {current_config}")
        ...
        try:
            ...
            if contrast_result.ok:
                ...
                log.warning(f"[texture-qa] QA failed on attempt {attempts}: {contrast_result.fails}")
                
                if attempts <= max_retries:
                    # Auto-dial back texture strength
                    current_config = _dial_back_texture_strength(current_config)
                    log.info(f"[texture-qa] Dialing back texture strength for retry: {current_config}")
                else:
                    log.warning(f"[texture-qa] Max retries reached, using last processed image")
                    
        except Exception as e:
            log.error(f"[texture-qa] Texture application failed on attempt {attempts}: {e}")
            qa_results.append({
                "attempt": attempts,
                "config": current_config.copy(),
                "error": str(e)
    ...
    log.info(f"[texture-qa] Texture application completed successfully after {attempts} attempts")
...
def _dial_back_texture_strength(config: Dict) -> Dict:
    ...
    if "grain_strength" in dialed_config:
        ...
        log.info(f"[texture-qa] Reduced grain_strength from {current_strength:.3f} to {dialed_config['grain_strength']:.3f}")
    ...
    if "posterize_levels" in dialed_config:
        ...
        log.info(f"[texture-qa] Increased posterize_levels from {current_levels} to {dialed_config['posterize_levels']}")
    ...
    if "feather_px" in dialed_config:
        ...
        log.info(f"[texture-qa] Reduced feather_px from {current_feather:.1f} to {dialed_config['feather_px']:.1f}")
...
def process_rasterized_with_texture_qa(
    ...
    try:
        ...
        if metadata["textures"]["applied"]:
            ...
            processed_path = str(base_path.parent / f"{base_path.stem}_textured{base_path.suffix}")
            processed_img.save(processed_path)
            
            log.info(f"[texture-integrate] Applied texture overlay to {raster_path} -> {processed_path}")
            return processed_path, metadata
        else:
            log.warning(f"[texture-integrate] Texture application failed, using original: {raster_path}")
