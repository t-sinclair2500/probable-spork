...
from .sdk import FPS, VIDEO_H, VIDEO_W, AnimType, BrandStyle, Keyframe, load_style
...
def make_text_clip(
    text: str, style: BrandStyle, kind: Literal["hook", "body", "lower_third"]
) -> ColorClip:
    """
    ...
    clip = ColorClip(
        size=(width, height), color=(255, 255, 255), duration=1.0  # White RGB
    )
...
def bg_gradient(
    style: BrandStyle, motion: Literal["slow_pan", "slow_zoom"], duration: float = 5.0
) -> ColorClip:
    """
    ...
    bg_clip = ColorClip(
        size=(VIDEO_W, VIDEO_H), color=(0, 100, 255), duration=duration  # Blue RGB
    )
...
def make_background_clip(bg_path: str, duration: float) -> VideoClip:
    ...
    try:
        ...
        return ColorClip(
            size=(VIDEO_W, VIDEO_H), color=(240, 240, 240), duration=duration
        )


def apply_keyframes(
    clip: VideoClip, keyframes: List[Keyframe], scene_duration: float
) -> VideoClip:
    """
