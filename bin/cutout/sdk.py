...
class BrandStyle(BaseModel):
    ...
    safe_margins_px: int = Field(
        default=SAFE_MARGINS_PX, description="Safe margins in pixels"
    )
    corner_radius: int = Field(default=8, description="Default corner radius")
    stroke: Dict[str, Union[str, int]] = Field(
        default_factory=dict, description="Stroke properties"
    )
    shadow: Dict[str, Union[str, int]] = Field(
        default_factory=dict, description="Shadow properties"
    )
    backgrounds: List[str] = Field(
        default_factory=list, description="Background options"
    )
    icon_palette: List[str] = Field(
        default_factory=list, description="Icon color options"
    )
...
class Element(BaseModel):
    ...
    asset_path: Optional[str] = Field(
        None, description="Path to asset file (SVG, PNG, etc.)"
    )
    ...
    keyframes: List[Keyframe] = Field(
        default_factory=list, description="Animation keyframes"
    )
    style: Optional[Dict[str, Union[str, int, float]]] = Field(
        None, description="Element-specific style"
    )
    metadata: Optional[Dict[str, Union[str, int, float]]] = Field(
        None, description="Element metadata"
    )
...
class Scene(BaseModel):
    ...
    metadata: Optional[Dict[str, Union[str, int, float]]] = Field(
        None, description="Scene metadata"
    )
...
def load_style(path: Optional[str] = None) -> BrandStyle:
    ...
    default_style = {
        ...
        "safe_margins_px": 64,
        "corner_radius": 8,
        "stroke": {"width": 2, "color": "#e5e7eb", "style": "solid"},
        "shadow": {
            "x_offset": 0,
            "y_offset": 4,
            "blur": 12,
    ...
    try:
        ...
        if not all(
            key in merged_data.get("font_sizes", {}) for key in required_font_sizes
        ):
            missing = required_font_sizes - set(
                merged_data.get("font_sizes", {}).keys()
            )
