...
from bin.cutout.sdk import validate_scene_script
...
def validate_scene(scene: Dict, scene_index: int) -> List[str]:
    ...
    if scene["duration_ms"] < min_scene_ms or scene["duration_ms"] > max_scene_ms:
        errors.append(
            f"Scene {scene_index}: duration {duration_sec:.1f}s outside {min_scene_ms/1000:.1f}-{max_scene_ms/1000:.1f}s bounds"
        )
...
def validate_element(element: Dict, scene_index: int, element_index: int) -> List[str]:
    ...
    for field in required_fields:
        if field not in element:
            errors.append(
                f"Scene {scene_index}, Element {element_index}: Missing required field: {field}"
            )
    ...
    if not isinstance(element["id"], str):
        errors.append(
            f"Scene {scene_index}, Element {element_index}: id must be a string"
        )
    ...
    if element["type"] not in valid_types:
        errors.append(
            f"Scene {scene_index}, Element {element_index}: type must be one of {valid_types}"
        )
    ...
    if element["type"] == "text" and "content" in element:
        ...
        if content and isinstance(content, str):
            ...
            if word_count > 12:  # MAX_WORDS_PER_CARD
                errors.append(
                    f"Scene {scene_index}, Element {element_index}: Text has {word_count} words, exceeds 12 word limit"
                )

    # Position validation
    if "x" in element and not isinstance(element["x"], (int, float)):
        errors.append(
            f"Scene {scene_index}, Element {element_index}: x must be a number"
        )

    if "y" in element and not isinstance(element["y"], (int, float)):
        errors.append(
            f"Scene {scene_index}, Element {element_index}: y must be a number"
        )
    ...
    if "keyframes" in element:
        ...
        if not isinstance(keyframes, list):
            errors.append(
                f"Scene {scene_index}, Element {element_index}: keyframes must be an array"
            )
        else:
            for k, keyframe in enumerate(keyframes):
                if not isinstance(keyframe, dict):
                    errors.append(
                        f"Scene {scene_index}, Element {element_index}, Keyframe {k}: Must be an object"
                    )
                    continue

                keyframe_errors = validate_keyframe(
                    keyframe, scene_index, element_index, k
                )
...
def validate_keyframe(
    keyframe: Dict, scene_index: int, element_index: int, keyframe_index: int
) -> List[str]:
    ...
    if "t" not in keyframe:
        errors.append(
            f"Scene {scene_index}, Element {element_index}, Keyframe {keyframe_index}: Missing required field: t"
        )
        return errors
    ...
    if not isinstance(keyframe["t"], int) or keyframe["t"] < 0:
        errors.append(
            f"Scene {scene_index}, Element {element_index}, Keyframe {keyframe_index}: t must be a non-negative integer"
        )
    ...
    if "opacity" in keyframe:
        ...
        if not isinstance(opacity, (int, float)) or opacity < 0 or opacity > 1:
            errors.append(
                f"Scene {scene_index}, Element {element_index}, Keyframe {keyframe_index}: opacity must be between 0.0 and 1.0"
            )
    ...
    if "scale" in keyframe and keyframe["scale"] is not None:
        ...
        if not isinstance(scale, (int, float)) or scale < 0:
            errors.append(
                f"Scene {scene_index}, Element {element_index}, Keyframe {keyframe_index}: scale must be non-negative"
            )
...
def main():
    ...
    parser.add_argument(
        "--in", dest="input_file", required=True, help="Input SceneScript JSON file"
    )
    ...
    try:
        ...
        if args.verbose:
            ...
            total_duration = (
                sum(scene.duration_ms for scene in scene_script.scenes) / 1000
            )
            ...
            scenes_within_bounds = sum(
                1
                for s in scene_script.scenes
                if min_scene_ms <= s.duration_ms <= max_scene_ms
            )
            compliance_percent = (
                (scenes_within_bounds / len(scene_script.scenes) * 100)
                if scene_script.scenes
                else 0
            )
            print(
                f"  Duration compliance: {compliance_percent:.1f}% within {min_scene_ms}-{max_scene_ms}ms bounds"
            )
            ...
            if truncation_count > 0:
                print(
                    f"  Text truncations: {truncation_count} elements exceed 12 word limit"
                )
            else:
                print("  Text compliance: All elements within 12 word limit")
