...
from bin.core import BASE, get_logger, load_config, load_modules_cfg, single_lock
...
class AcceptanceValidator:
    ...
    def __init__(self, cfg):
        ...
        self.results = {
            ...
            "lanes": {"youtube": {"status": "PENDING", "artifacts": {}, "quality": {}}},
            "quality_thresholds": {
                "script_score_min": self.render_cfg.get("acceptance", {}).get(
                    "min_script_score", 50
                ),
                "seo_lint_pass": True,
                "visual_coverage_min": self.render_cfg.get("acceptance", {}).get(
                    "min_coverage", 0.85
                )
                * 100,
                "min_assets": self.render_cfg.get("acceptance", {}).get(
                    "min_assets", 10
                ),
                "min_script_words": self.render_cfg.get("acceptance", {}).get(
            ...
            "acceptance_config": {
                "duration_tolerance_pct": self.render_cfg.get("acceptance", {}).get(
                    "tolerance_pct", 5.0
                ),
                "audio_validation_required": self.render_cfg.get("acceptance", {}).get(
                    "audio_validation_required", True
                ),
                "caption_validation_required": self.render_cfg.get(
                    "acceptance", {}
                ).get("caption_validation_required", True),
                "legibility_validation_required": self.render_cfg.get(
                    "acceptance", {}
                ).get("legibility_validation_required", True),
                "wcag_aa_threshold": self.render_cfg.get("acceptance", {}).get(
    ...
    def _validate_assets_quality(self, artifacts: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not slug:
            ...
            asset_results["errors"].append(
                "Could not determine slug for asset validation"
            )
        ...
        try:
            ...
            if len(gaps) > 0:
                asset_results["status"] = "FAIL"
                asset_results["errors"].append(
                    f"Found {len(gaps)} unresolved asset gaps"
                )
            ...
            if os.path.exists(library_manifest_path):
                try:
                    ...
                    if total_assets > 50:
                        ...
                        reuse_threshold = getattr(self.modules_cfg, "assets", {}).get(
                            "reuse_ratio_threshold", 0.7
                        )
                        if reuse_ratio < reuse_threshold:
                            asset_results["status"] = "FAIL"
                            asset_results["errors"].append(
                                f"Reuse ratio {reuse_ratio:.1%} below threshold {reuse_threshold:.1%}"
                            )
                        else:
                            asset_results["reuse_ratio"]["status"] = "pass"
                    else:
                        asset_results["reuse_ratio"]["status"] = "exempt"
                        asset_results["warnings"].append(
                            f"Library has only {total_assets} assets, reuse ratio not enforced"
                        )
                except Exception as e:
                    log.warning(
                        f"[acceptance-assets] Could not read library manifest: {e}"
                    )
            ...
            log.info(
                f"[acceptance-assets] Asset validation completed: {asset_results['status']}"
            )
            return asset_results
    ...
    def _check_palette_compliance(
        self, asset_plan: Dict[str, Any], slug: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if os.path.exists(library_manifest_path):
                ...
                for asset in resolved_assets:
                    ...
                    if asset_hash and asset_hash in manifest.get("assets", {}):
                        ...
                        if palette_ok:
                            ...
                            delta_e_violations = manifest_asset.get(
                                "delta_e_violations", []
                            )
                ...
                generation_report_path = os.path.join(
                    BASE, "runs", slug, "asset_generation_report.json"
                )
                ...
                log.warning(
                    "[acceptance-assets] Library manifest not found, using basic palette check"
                )
            ...
            if result["violations"]:
                result["status"] = "FAIL"
                log.warning(
                    f"[acceptance-assets] Found {len(result['violations'])} palette violations"
                )
            else:
                result["status"] = "pass"
                log.info(
                    f"[acceptance-assets] All {result['total_assets']} assets are palette compliant"
                )
    ...
    def _check_qa_results(self, slug: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for scene in scenes:
                ...
                if collisions:
                    result["collisions"].extend(
                        [{"scene_id": scene_id, "collisions": collisions}]
                    )
                ...
                if violations:
                    result["safe_margin_violations"].extend(
                        [{"scene_id": scene_id, "violations": violations}]
                    )
                ...
                if contrast.get("status") == "fail":
                    result["contrast_issues"].append(
                        {"scene_id": scene_id, "contrast": contrast}
                    )
            ...
            if (
                result["collisions"]
                or result["safe_margin_violations"]
                or result["contrast_issues"]
            ):
                result["status"] = "FAIL"
    ...
    def _extract_provenance(self, slug: str) -> List[Dict[str, Any]]:
        ...
        if os.path.exists(generation_report_path):
            try:
                ...
                log.info(
                    f"[acceptance-assets] Found {len(provenance)} generated assets with provenance"
                )

            except Exception as e:
                log.warning(
                    f"[acceptance-assets] Could not read generation report: {e}"
                )
    ...
    def _extract_slug_from_artifacts(self, artifacts: Dict[str, Any]) -> Optional[str]:
        ...
        if script:
            ...
            if (
                "_" in slug
                and len(slug.split("_")[0]) == 10
                and slug.split("_")[0].replace("-", "").isdigit()
            ):
    ...
    def _update_video_metadata_with_assets(
        self, slug: str, asset_results: Dict[str, Any]
    ):
        ...
        if not os.path.exists(video_metadata_path):
            log.warning(
                f"[acceptance-assets] Video metadata not found: {video_metadata_path}"
            )
            return
        ...
        try:
            ...
            metadata["assets"].update(
                {
                    "coverage_pct": (
                        100.0 if asset_results["coverage"]["gaps_count"] == 0 else 0.0
                    ),
                    ...
                    "palette_names_used": list(
                        set(
                            [
                                color
                                for asset in asset_results["provenance"]
                                for color in asset.get("palette", [])
                            ]
                        )
                    ),
            ...
            log.info(
                "[acceptance-assets] Updated video metadata with asset information"
            )

        except Exception as e:
            log.error(f"[acceptance-assets] Failed to update video metadata: {e}")

    def _update_video_metadata_with_evidence(
        self, slug: str, evidence_results: Dict[str, Any]
    ):
        """Update video metadata with evidence and research quality information"""
        log.info(
            f"[acceptance-evidence] Updating video metadata with evidence information for {slug}"
        )

        video_metadata_path = os.path.join(BASE, "videos", f"{slug}.metadata.json")
        if not os.path.exists(video_metadata_path):
            log.warning(
                f"[acceptance-evidence] Video metadata not found: {video_metadata_path}"
            )
            return
        ...
        try:
            ...
            metadata["citations"].update(
                {
                    ...
                    "beats_coverage_pct": evidence_results["citations"][
                        "beats_coverage_pct"
                    ],
                    "fact_guard_summary": {
                        "removed": evidence_results["fact_guard_summary"][
                            "removed_count"
                        ],
                        "rewritten": evidence_results["fact_guard_summary"][
                            "rewritten_count"
                        ],
                        "flagged": evidence_results["fact_guard_summary"][
                            "flagged_count"
                        ],
                        "strict_mode_fail": evidence_results["fact_guard_summary"][
                            "strict_mode_fail"
                        ],
                    },
                    "policy_compliance": {
                        "min_citations_per_intent": evidence_results["policy_checks"][
                            "min_citations_per_intent"
                        ],
                        "whitelist_compliance": evidence_results["policy_checks"][
            ...
            log.info(
                "[acceptance-evidence] Updated video metadata with evidence information"
            )

        except Exception as e:
            log.error(
                f"[acceptance-evidence] Failed to update video metadata with evidence: {e}"
            )

    def _update_video_metadata_with_pacing(
        self, slug: str, pacing_results: Dict[str, Any]
    ):
        """Update video metadata with pacing validation information"""
        log.info(
            f"[acceptance-pacing] Updating video metadata with pacing information for {slug}"
        )

        video_metadata_path = os.path.join(BASE, "videos", f"{slug}.metadata.json")
        if not os.path.exists(video_metadata_path):
            log.warning(
                f"[acceptance-pacing] Video metadata not found: {video_metadata_path}"
            )
            return
        ...
        try:
            ...
            log.info(
                "[acceptance-pacing] Updated video metadata with pacing validation information"
            )

        except Exception as e:
            log.error(
                f"[acceptance-pacing] Failed to update video metadata with pacing: {e}"
            )
    ...
    def validate_youtube_lane(self) -> bool:
        ...
        if self.modules_cfg and self.modules_cfg.get("animatics", {}).get(
            "enabled", False
        ):
            animatics_enabled = True
        elif getattr(self.cfg, "animatics", None) and getattr(
            self.cfg.animatics, "enabled", True
        ):
            animatics_enabled = True
        ...
        if animatics_enabled:
            ...
            if not animatics_quality.get("coverage_pass", False):
                youtube_results["status"] = "FAIL"
                youtube_results["quality"][
                    "error"
                ] = "Animatics coverage below threshold"
                return False
            ...
            if animatics_only and not enable_legacy:
                ...
                if artifacts["assets"] and not artifacts.get("animatics"):
                    ...
                    youtube_results["quality"][
                        "error"
                    ] = "Stock assets present in animatics_only mode"
                    return False
                ...
                if artifacts["video_metadata"]:
                    try:
                        ...
                        if source_mode != "animatics":
                            ...
                            youtube_results["quality"][
                                "error"
                            ] = f"Video source_mode is '{source_mode}', expected 'animatics' in animatics_only mode"
                            return False
                        ...
                        if isinstance(coverage, dict):
                            ...
                            min_coverage = (
                                getattr(self.cfg.video, "min_coverage", 0.85) * 100
                            )
                            if coverage_pct < min_coverage:
                                youtube_results["status"] = "FAIL"
                                youtube_results["quality"][
                                    "error"
                                ] = f"Coverage {coverage_pct}% below threshold {min_coverage}%"
                                return False
                    except Exception as e:
                        log.warning(f"Could not validate video metadata: {e}")
                        youtube_results["quality"][
                            "warning"
                        ] = "Could not validate video metadata"
        ...
        if (
            quality.get("script_score", 0)
            < self.results["quality_thresholds"]["script_score_min"]
        ):
            youtube_results["status"] = "FAIL"
            youtube_results["quality"][
                "error"
            ] = f"Script score {quality.get('script_score', 0)} below threshold {self.results['quality_thresholds']['script_score_min']}"
            return False

        # Asset count validation - adjust for animatics-only mode
        min_assets_required = self.results["quality_thresholds"]["min_assets"]
        if hasattr(self.cfg, "video") and getattr(
            self.cfg.video, "animatics_only", False
        ):
        ...
        if len(artifacts["assets"]) < min_assets_required:
            ...
            youtube_results["quality"][
                "error"
            ] = f"Only {len(artifacts['assets'])} assets, need {min_assets_required}"
            return False

        if (
            quality.get("script_words", 0)
            < self.results["quality_thresholds"]["min_script_words"]
        ):
            youtube_results["status"] = "FAIL"
            youtube_results["quality"][
                "error"
            ] = f"Script only {quality.get('script_words', 0)} words, need {self.results['quality_thresholds']['min_script_words']}"
            return False
        ...
        if not audio_quality.get("valid", False):
            ...
            youtube_results["quality"][
                "error"
            ] = f"Audio validation failed: {audio_error}"
            ...
            if asset_quality.get("status") == "FAIL":
                ...
                asset_error_msg = (
                    f"Asset validation also failed: {'; '.join(asset_errors[:3])}"
                )
        ...
        caption_required = self.render_cfg.get("acceptance", {}).get(
            "caption_validation_required", True
        )
        caption_blocks = self.render_cfg.get("acceptance", {}).get(
            "caption_failure_blocks", False
        )

        if (
            not caption_validation.get("valid", False)
            and caption_required
            and caption_blocks
        ):
            ...
            youtube_results["quality"][
                "error"
            ] = f"Caption validation failed: {caption_error}"
            return False
        ...
        legibility_required = self.render_cfg.get("acceptance", {}).get(
            "legibility_validation_required", True
        )
        if not legibility_validation.get("valid", False) and legibility_required:
            legibility_error = legibility_validation.get(
                "error", "Legibility validation failed"
            )
            youtube_results["status"] = "FAIL"
            youtube_results["quality"][
                "error"
            ] = f"Legibility validation failed: {legibility_error}"
            return False
        ...
        if os.path.exists(scripts_dir):
            ...
            if script_files:
                ...
                if os.path.exists(animatics_dir):
                    animatics_files = [
                        f for f in os.listdir(animatics_dir) if f.endswith(".mp4")
                    ]
                    ...
                    if hasattr(self.cfg, "video") and getattr(
                        ...
                        log.info(
                            f"Animatics-only mode: using {len(animatics_files)} animatic scenes as assets"
                        )
    ...
    def _validate_youtube_audio(self, artifacts: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            from bin.audio_validator import (
                validate_audio_for_acceptance,
                validate_ducking_for_acceptance,
            )

            # Check if audio validation is required
            audio_required = self.render_cfg.get("acceptance", {}).get(
                "audio_validation_required", True
            )
            ...
            if artifacts["video"]:
                ...
                if os.path.exists(video_path):
                    ...
                    temp_audio = os.path.join(
                        self.temp_dir, f"temp_audio_{os.path.basename(video_path)}.mp3"
                    )
                    ...
                    try:
                        ...
                        if os.path.exists(temp_audio):
                            ...
                            mixed_validation = validate_audio_for_acceptance(
                                temp_audio, "mixed"
                            )
            ...
            for audio_type, validation in audio_quality.items():
                if isinstance(validation, dict) and not validation.get("valid", False):
                    ...
                    error_msg = validation.get(
                        "error", f"{audio_type} validation failed"
                    )
        ...
        try:
            ...
            if artifacts["scenescript"]:
                scenescript_path = os.path.join(
                    BASE, "scenescripts", artifacts["scenescript"]
                )
                if os.path.exists(scenescript_path):
                    ...
                    max_words = (
                        getattr(
                            getattr(self.cfg, "animatics", None),
                            "max_words_per_card",
                            12,
                        )
                        if hasattr(self.cfg, "animatics")
                        else 12
                    )
                    safe_margins = (
                        getattr(
                            getattr(self.cfg, "animatics", None), "safe_margins_px", 80
                        )
                        if hasattr(self.cfg, "animatics")
                        else 80
                    )

                    legibility_issues = self._check_scenescript_legibility(
                        scenescript_data, max_words, safe_margins
                    )
                    ...
                    min_coverage = (
                        getattr(
                            getattr(self.cfg, "animatics", None), "min_coverage", 0.85
                        )
                        if hasattr(self.cfg, "animatics")
                        else 0.85
                    )
                    ...
                    if duration_result.get("details"):
                        ...
                        metadata_path = self._write_video_metadata(
                            slug, duration_result["details"]
                        )
                        if metadata_path:
                            quality["metadata_path"] = metadata_path

                    # Validate layout engine integration
                    layout_result = self._validate_layout_engine_integration(
                        scenescript_data
                    )
                    ...
                    quality["overall_pass"] = (
                        quality.get("validation_pass", False)
                        and quality.get("legibility_pass", False)
                        and quality.get("coverage_pass", False)
                        and quality.get("qa_gates_pass", False)
                        and quality.get("duration_pass", False)
                        and quality.get("layout_pass", False)
                    )
    ...
    def _check_scenescript_legibility(
        self, scenescript_data: Dict, max_words: int, safe_margins: int
    ) -> List[str]:
            ...
            for i, scene in enumerate(scenes):
                ...
                for j, element in enumerate(elements):
                    if element.get("type") == "text" and element.get("content"):
                        ...
                        if words > max_words:
                            issues.append(
                                f"Scene {i}, Element {j}: {words} words exceeds limit of {max_words}"
                            )
                        ...
                        if x < safe_margins or y < safe_margins:
                            issues.append(
                                f"Scene {i}, Element {j}: Text too close to left/top edge"
                            )

                        # Check if text extends beyond safe right/bottom bounds
                        if (
                            x + width > 1920 - safe_margins
                            or y + height > 1080 - safe_margins
                        ):
                            issues.append(
                                f"Scene {i}, Element {j}: Text extends beyond safe right/bottom bounds"
                            )
    ...
    def _validate_duration_policy(self, scenescript_data: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.render_cfg:
                ...
                log.info(
                    f"[duration-policy] Using render.yaml timing config: tolerance={timing_config['target_tolerance_pct']}%"
                )
            else:
                try:
                    ...
                    log.info(
                        f"[duration-policy] Using modules.yaml timing config: tolerance={timing_config.get('target_tolerance_pct', 5.0)}%"
                    )
                except Exception as e:
                    ...
                    log.info(
                        f"[duration-policy] Using default timing config: tolerance={timing_config['target_tolerance_pct']}%"
                    )
            ...
            for i, scene in enumerate(scenes):
                ...
                if duration < min_scene_ms or duration > max_scene_ms:
                    out_of_bounds_scenes.append(
                        f"Scene {i}: {duration}ms (bounds: {min_scene_ms}-{max_scene_ms}ms)"
                    )

            # Check total duration tolerance
            tolerance_pct = timing_config.get("target_tolerance_pct", 5.0)
            deviation_pct = (
                abs(total_duration_ms - target_ms) / target_ms * 100
                if target_ms > 0
                else 0
            )
            ...
            try:
                ...
                if vo_cues and vo_cues.get("scenes"):
                    ...
                    vo_deviation_pct = (
                        abs(vo_total_ms - target_ms) / target_ms * 100
                        if target_ms > 0
                        else 0
                    )
            ...
            log.info(
                f"[duration-policy] Duration policy validation: {'PASS' if result['pass'] else 'FAIL'}"
            )
            log.info(
                f"[duration-policy] Target: {target_ms}ms, Actual: {total_duration_ms}ms, Deviation: {deviation_pct:.1f}%"
            )
        ...
        try:
            ...
            metadata = {
                "duration": {
                    ...
                    "source": (
                        "brief"
                        if duration_details.get("within_tolerance", False)
                        else "calculated"
                    ),
                },
                ...
                "validation": {
                    ...
                    "scene_bounds_pass": duration_details.get(
                        "scene_bounds_pass", False
                    ),
            ...
            if vo_details:
                ...
                metadata["duration"]["vo_deviation_pct"] = vo_details.get(
                    "vo_deviation_pct", 0
                )
    ...
    def _run_qa_gates(self, scenescript_data: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            for scene in scenes:
                ...
                if (
                    "composition_rules" in scene
                    and "background_color" in scene["composition_rules"]
                ):
            ...
            return {
                "ok": False,
                "fails": ["QA gates module not available"],
                "warnings": [],
                "details": {},
            }
        except Exception as e:
        ...
        try:
            ...
            for scene in scenes:
                ...
                if elements:
                    ...
                    has_text = any(
                        e.get("type") == "text" and e.get("content") for e in elements
                    )
                    has_visuals = any(
                        e.get("type") in ["prop", "character", "shape"]
                        for e in elements
                    )
    ...
    def _validate_layout_engine_integration(
        self, scenescript_data: Dict
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            animatics_only = (
                getattr(self.cfg.video, "animatics_only", True)
            ...
            if result["pass"]:
                log.info(
                    f"[layout] Layout validation PASSED: {positioned_elements}/{total_elements} elements positioned"
                )
            else:
                log.warning(
                    f"[layout] Layout validation FAILED: {len(result['issues'])} issues found"
                )
    ...
    def _validate_visual_polish(self, artifacts: Dict[str, Any]) -> Dict[str, Any]:
        ...
        visual_polish_results = {
            ...
            "performance": {
                "status": "PENDING",
                "texture_delta_pct": 0.0,
                "render_time_with": 0.0,
                "render_time_without": 0.0,
            },
            "errors": [],
            "warnings": [],
        }
        ...
        if not slug:
            ...
            visual_polish_results["errors"].append(
                "Could not determine slug for visual polish validation"
            )
        ...
        if textures_enabled:
            ...
            if artifacts.get("video_metadata"):
                try:
                    ...
                    log.info(
                        f"[acceptance-visual-polish] Reading video metadata from: {metadata_path}"
                    )
                    ...
                    log.info(
                        f"[acceptance-visual-polish] Video metadata keys: {list(metadata.keys())}"
                    )

                    texture_metadata = metadata.get("textures", {})
                    if texture_metadata:
                        log.info(
                            f"[acceptance-visual-polish] Found texture metadata: {texture_metadata}"
                        )
                        ...
                        contrast_status = metadata.get("legibility", {}).get(
                            "contrast_status", "UNKNOWN"
                        )
                        visual_polish_results["textures"][
                            "contrast_check"
                        ] = contrast_status

                        if contrast_status != "PASS":
                            visual_polish_results["warnings"].append(
                                f"Texture applied but contrast status is {contrast_status}"
                            )
                    else:
                        log.warning(
                            "[acceptance-visual-polish] No texture metadata found in video metadata"
                        )
                        visual_polish_results["textures"]["status"] = "FAIL"
                        visual_polish_results["errors"].append(
                            "Textures enabled but no texture metadata found"
                        )
                except Exception as e:
                    log.warning(
                        f"Could not read video metadata for texture validation: {e}"
                    )
                    visual_polish_results["textures"]["status"] = "WARN"
                    visual_polish_results["warnings"].append(
                        f"Could not validate texture metadata: {e}"
                    )

            # Check for texture probe grid
            texture_probe_path = os.path.join(
                BASE, "runs", slug, "texture_probe_grid.png"
            )
        ...
        geometry_enabled = (
            self.modules_cfg.get("geometry", {}).get("enable", False)
            if self.modules_cfg
            else False
        )
        ...
        if geometry_enabled:
            ...
            geom_report_path = os.path.join(
                BASE, "runs", slug, "geom_validation_report.json"
            )
            if os.path.exists(geom_report_path):
                try:
                    ...
                    visual_polish_results["geometry"][
                        "critical_errors"
                    ] = critical_errors
                    ...
                    if critical_errors == 0:
                        ...
                        visual_polish_results["errors"].append(
                            f"Geometry validation has {critical_errors} critical errors"
                        )
                except Exception as e:
                    log.warning(f"Could not read geometry validation report: {e}")
                    visual_polish_results["geometry"]["status"] = "WARN"
                    visual_polish_results["warnings"].append(
                        f"Could not validate geometry report: {e}"
                    )
            else:
                visual_polish_results["geometry"]["status"] = "FAIL"
                visual_polish_results["errors"].append(
                    "Geometry enabled but validation report not found"
                )

        # Check micro-animations settings and validation
        micro_anim_enabled = (
            self.modules_cfg.get("micro_anim", {}).get("enable", False)
            if self.modules_cfg
            else False
        )
        ...
        if micro_anim_enabled:
            ...
            style_signature_path = os.path.join(
                BASE, "runs", slug, "style_signature.json"
            )
            if os.path.exists(style_signature_path):
                try:
                    ...
                    visual_polish_results["micro_animations"][
                        "elements_animated"
                    ] = elements_animated
                    visual_polish_results["micro_animations"][
                        "total_elements"
                    ...
                    if total_elements > 0:
                        ...
                        if animation_percent <= 10:
                            ...
                            visual_polish_results["errors"].append(
                                f"Animation percentage {animation_percent:.1f}% exceeds 10% limit"
                            )
                    else:
                        visual_polish_results["micro_animations"]["status"] = "WARN"
                        visual_polish_results["warnings"].append(
                            "No elements found for animation validation"
                        )
                    ...
                    if collision_status == "PASS":
                        ...
                        visual_polish_results["micro_animations"]["status"] = "FAIL"
                        visual_polish_results["errors"].append(
                            "Collision check failed after animation keyframes"
                        )
                    else:
                        visual_polish_results["warnings"].append(
                            f"Collision check status unclear: {collision_status}"
                        )

                except Exception as e:
                    log.warning(
                        f"Could not read style signature for micro-animation validation: {e}"
                    )
                    visual_polish_results["micro_animations"]["status"] = "WARN"
                    visual_polish_results["warnings"].append(
                        f"Could not validate micro-animations: {e}"
                    )
            else:
                visual_polish_results["micro_animations"]["status"] = "WARN"
                visual_polish_results["warnings"].append(
                    "Style signature not found for micro-animation validation"
                )
        ...
        if textures_enabled:
            ...
            if artifacts.get("video_metadata"):
                try:
                    ...
                    render_time_with = performance_metrics.get(
                        "render_time_with_textures", 0.0
                    )
                    render_time_without = performance_metrics.get(
                        "render_time_without_textures", 0.0
                    )

                    if render_time_with > 0 and render_time_without > 0:
                        delta_pct = (
                            (render_time_with - render_time_without)
                            / render_time_without
                        ) * 100
                        visual_polish_results["performance"][
                            "texture_delta_pct"
                        ] = delta_pct
                        visual_polish_results["performance"][
                            "render_time_with"
                        ...
                        if delta_pct <= 15:
                            ...
                            visual_polish_results["warnings"].append(
                                f"Texture render time increase {delta_pct:.1f}% exceeds 15% threshold"
                            )
                    else:
                        visual_polish_results["performance"]["status"] = "UNKNOWN"
                        visual_polish_results["warnings"].append(
                            "Performance metrics not available for texture comparison"
                        )
                    ...
                    visual_polish_results["performance"]["status"] = "UNKNOWN"
                    visual_polish_results["warnings"].append(
                        f"Could not validate performance: {e}"
                    )

        # Determine overall visual polish status
        all_passes = (
            visual_polish_results["textures"]["status"] in ["PASS", "WARN", "UNKNOWN"]
            and visual_polish_results["geometry"]["status"]
            in ["PASS", "WARN", "UNKNOWN"]
            and visual_polish_results["micro_animations"]["status"]
            in ["PASS", "WARN", "UNKNOWN"]
            and visual_polish_results["performance"]["status"]
            in ["PASS", "WARN", "UNKNOWN"]
        )

        has_critical_failures = (
            visual_polish_results["textures"]["status"] == "FAIL"
            or visual_polish_results["geometry"]["status"] == "FAIL"
            or visual_polish_results["micro_animations"]["status"] == "FAIL"
        )
        ...
        log.info(
            f"[acceptance-visual-polish] Visual polish validation completed: {visual_polish_results['status']}"
        )
        return visual_polish_results
    ...
    def _validate_evidence_quality(self, artifacts: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not slug:
            ...
            evidence_results["errors"].append(
                "Could not determine slug for evidence validation"
            )
        ...
        try:
            ...
            grounded_beats_path = os.path.join(
                BASE, "data", slug, "grounded_beats.json"
            )
            ...
            if not os.path.exists(references_path):
                evidence_results["errors"].append(
                    f"Missing references.json: {references_path}"
                )
            if not os.path.exists(grounded_beats_path):
                evidence_results["errors"].append(
                    f"Missing grounded_beats.json: {grounded_beats_path}"
                )
            if not os.path.exists(fact_guard_path):
                evidence_results["errors"].append(
                    f"Missing fact_guard_report.json: {fact_guard_path}"
                )
            ...
            evidence_results["fact_guard_summary"]["removed_count"] = (
                fact_guard_summary.get("removed", 0)
            )
            evidence_results["fact_guard_summary"]["rewritten_count"] = (
                fact_guard_summary.get("rewritten", 0)
            )
            evidence_results["fact_guard_summary"]["flagged_count"] = (
                fact_guard_summary.get("flagged", 0)
            )

            # Check strict mode failure (flagged > 0 in strict mode)
            strictness = fact_guard_report.get("metadata", {}).get(
                "strictness", "balanced"
            )
            if strictness == "strict" and fact_guard_summary.get("flagged", 0) > 0:
                ...
                evidence_results["status"] = "FAIL"
                evidence_results["errors"].append(
                    "Fact-guard strict mode: flagged claims found"
                )
            ...
            if os.path.exists(outline_path):
                try:
                    ...
                    intent_templates_path = os.path.join(
                        BASE, "conf", "intent_templates.yaml"
                    )
                    if os.path.exists(intent_templates_path):
                        ...
                        intent_config = intent_templates.get("intents", {}).get(
                            intent_type, {}
                        )
                        ...
                        # Validate coverage
                        if coverage_pct < min_coverage:
                            evidence_results["policy_checks"][
                                "min_citations_per_intent"
                            ] = "FAIL"
                            evidence_results["status"] = "FAIL"
                            evidence_results["errors"].append(
                        ...
                        avg_citations = (
                            total_citations / total_beats if total_beats > 0 else 0
                        )
                        ...
                        evidence_results["warnings"].append(
                            "Intent templates config not found, skipping citation policy validation"
                        )
                except Exception as e:
                    log.warning(
                        f"[acceptance-evidence] Could not validate intent citation requirements: {e}"
                    )
                    evidence_results["warnings"].append(
                        f"Intent validation error: {str(e)}"
                    )
            ...
            try:
                ...
                allowlist = set(
                    research_config.domains.get("allowlist", [])
                    if hasattr(research_config, "domains")
                    else []
                )
                blacklist = set(
                    research_config.domains.get("blacklist", [])
                    if hasattr(research_config, "domains")
                    else []
                )
                ...
                if allowlist:  # Only validate if allowlist is configured
                    ...
                    if non_whitelisted_domains:
                        ...
                        evidence_results["warnings"].append(
                            f"Found {len(non_whitelisted_domains)} non-whitelisted domains: {', '.join(non_whitelisted_domains[:5])}"
                        )
                    else:
                        evidence_results["policy_checks"][
                            "whitelist_compliance"
                    ...
                    evidence_results["warnings"].append(
                        "Domain allowlist not configured, skipping whitelist validation"
                    )
            except Exception as e:
                evidence_results["policy_checks"]["whitelist_compliance"] = "PENDING"
                evidence_results["warnings"].append(
                    f"Whitelist validation error: {str(e)}"
                )

            # Determine overall evidence status
            if (
                evidence_results["status"] == "PENDING"
                and not evidence_results["errors"]
            ):
                evidence_results["status"] = "PASS"

            log.info(
                f"[acceptance-evidence] Evidence validation completed: {evidence_results['status']}"
            )
            return evidence_results
    ...
    def _validate_pacing(self, artifacts: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not self.modules_cfg or not self.modules_cfg.get("pacing", {}).get(
            "enable", False
        ):
            pacing_results["status"] = "SKIP"
            pacing_results["warnings"].append(
                "Pacing validation disabled in modules.yaml"
            )
        ...
        if not slug:
            ...
            pacing_results["errors"].append(
                "Could not determine slug for pacing validation"
            )
            return pacing_results
        ...
        try:
            ...
            if not os.path.exists(pacing_report_path):
                pacing_results["status"] = "FAIL"
                pacing_results["errors"].append(
                    f"Missing pacing report: {pacing_report_path}"
                )
                return pacing_results
            ...
            if not kpi_metrics or not comparison:
                ...
                pacing_results["errors"].append(
                    "Missing KPI metrics or comparison data in pacing report"
                )
                return pacing_results
            ...
            missing_kpis = [
                ...
                if kpi not in kpi_metrics or kpi_metrics[kpi] is None
            ]
            if missing_kpis:
                pacing_results["status"] = "FAIL"
                pacing_results["verdict"] = "FAIL"
                pacing_results["errors"].append(
                    f"Missing required KPI metrics: {', '.join(missing_kpis)}"
                )
                return pacing_results
            ...
            if not out_of_band_metrics:
                ...
                if strict_mode:
                    ...
                    pacing_results["status"] = "FAIL"
                    pacing_results["errors"].append(
                        f"Metrics out of band in strict mode: {', '.join(out_of_band_metrics)}"
                    )
                else:
                    pacing_results["verdict"] = "WARN"
                    pacing_results["status"] = "WARN"
                    pacing_results["warnings"].append(
                        f"Metrics out of band (non-strict mode): {', '.join(out_of_band_metrics)}"
                    )
            ...
            log.info(
                f"[acceptance-pacing] Pacing validation completed: {pacing_results['status']} - {pacing_results['verdict']}"
            )
            return pacing_results
    ...
    def _validate_captions(self, artifacts: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if artifacts.get("captions"):
                log.info(
                    f"[acceptance] Found existing captions: {artifacts['captions']}"
                )
                return {
                    "valid": True,
                    "source": "existing",
            ...
            fallback_enabled = self.render_cfg.get("captions", {}).get(
                ...
                if artifacts.get("outline"):
                    ...
                    if os.path.exists(outline_path):
                        try:
                            ...
                            log.warning(
                                f"[acceptance] Could not read outline for intent: {e}"
                            )

                # Generate SRT
                srt_result = generate_srt_for_acceptance(
                    script_path=script_path, intent_type=intent_type
                )

                if srt_result["success"]:
                    log.info(
                        f"[acceptance] SRT generated successfully: {srt_result['generation_method']}"
                    )
                    return {
                        "valid": True,
                        "source": "generated",
    ...
    def _validate_legibility(self, artifacts: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not artifacts.get("scenescript"):
                log.info(
                    "[acceptance] No SceneScript found, skipping legibility validation"
                )
                return {
                    "valid": True,
                    "skipped": True,
            ...
            try:
                ...
                if legibility_result["valid"]:
                    log.info(
                        f"[acceptance] Legibility validation passed: {legibility_result['summary']}"
                    )
                else:
                    log.warning(
                        f"[acceptance] Legibility validation failed: {legibility_result['summary']}"
                    )
    ...
    def run_validation(self) -> Dict[str, Any]:
        ...
        evidence_ok = self._validate_evidence_quality(
            self.results["lanes"]["youtube"]["artifacts"]
        )
        ...
        slug = self._extract_slug_from_artifacts(
            self.results["lanes"]["youtube"]["artifacts"]
        )
        if slug:
        ...
        pacing_has_kpis = (
            pacing_ok.get("kpi_metrics")
            and pacing_ok.get("comparison")
            and pacing_ok.get("kpi_metrics").get("words_per_sec") is not None
        )

        if not pacing_has_kpis:
            log.warning(
                "[acceptance-pacing] Pacing integrity guard: Missing KPI metrics or comparison data"
            )
            pacing_ok["status"] = "FAIL"
            pacing_ok["verdict"] = "FAIL"
            pacing_ok["errors"].append(
                "Pacing integrity guard: Missing KPI metrics or comparison data"
            )
            self.results["pacing"] = pacing_ok

        if (
            youtube_ok
            and evidence_ok["status"] == "PASS"
            and pacing_ok["status"] == "PASS"
        ):
        ...
        if "quality" in self.results["lanes"]["youtube"]:
            assets_quality = self.results["lanes"]["youtube"]["quality"].get(
                "assets", {}
            )
            if assets_quality:
                self.results["assets"].update(assets_quality)
                log.info(
                    f"[acceptance-assets] Updated main assets section with status: {assets_quality.get('status', 'unknown')}"
                )

            # Update visual polish validation status from YouTube lane
            visual_polish_quality = self.results["lanes"]["youtube"]["quality"].get(
                "visual_polish", {}
            )
            if visual_polish_quality:
                self.results["visual_polish"].update(visual_polish_quality)
                log.info(
                    f"[acceptance-visual-polish] Updated main visual polish section with status: {visual_polish_quality.get('status', 'unknown')}"
                )
        ...
        self.results["summary"] = {
            ...
            "total_artifacts": {
                "youtube": len(
                    [
                        v
                        for v in self.results["lanes"]["youtube"]["artifacts"].values()
                        if v
                    ]
                ),
            },
        ...
        determinism_required = self.render_cfg.get("acceptance", {}).get(
        ...
        if determinism_required:
            ...
            log.info(
                f"[acceptance] Determinism validation: {determinism_validation['status']}"
            )

        # Add acceptance configuration summary
        self.results["acceptance_config_summary"] = {
            "duration_tolerance_pct": self.results["acceptance_config"][
                "duration_tolerance_pct"
            ],
            "audio_validation_required": self.results["acceptance_config"][
                "audio_validation_required"
            ],
            "caption_validation_required": self.results["acceptance_config"][
                "caption_validation_required"
            ],
            "legibility_validation_required": self.results["acceptance_config"][
                "legibility_validation_required"
            ],
    ...
    def _validate_determinism(self) -> Dict[str, Any]:
        ...
        try:
            ...
            current_metrics = {
                ...
                "evidence_status": self.results.get("evidence", {}).get(
                    "status", "UNKNOWN"
                ),
                "pacing_status": self.results.get("pacing", {}).get(
                    "status", "UNKNOWN"
                ),
                "duration_tolerance_pct": self.results["acceptance_config"][
                    "duration_tolerance_pct"
                ],
                "wcag_aa_threshold": self.results["acceptance_config"][
                    "wcag_aa_threshold"
                ],
            }
            ...
            previous_metrics = {
                ...
                "youtube_lane_status": previous_results.get("lanes", {})
                .get("youtube", {})
                .get("status", "UNKNOWN"),
                "evidence_status": previous_results.get("evidence", {}).get(
                    "status", "UNKNOWN"
                ),
                "pacing_status": previous_results.get("pacing", {}).get(
                    "status", "UNKNOWN"
                ),
                "duration_tolerance_pct": previous_results.get(
                    "acceptance_config_summary", {}
                ).get("duration_tolerance_pct", 0),
                "wcag_aa_threshold": previous_results.get(
                    "acceptance_config_summary", {}
                ).get("wcag_aa_threshold", 0),
            }
            ...
            if deterministic:
                log.info(
                    "[acceptance] Determinism validation PASSED - results are consistent"
                )
            else:
                log.warning(
                    f"[acceptance] Determinism validation FAILED - {len(differences)} differences found"
                )
                for diff in differences:
                    log.warning(
                        f"[acceptance] Difference: {diff['metric']} = {diff['current']} (was {diff['previous']})"
                    )
    ...
    def _check_srt_consistency(
        self, previous_results: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Check if SRT files are consistent across runs"""
        try:
            # Get current SRT info
            current_captions = self.results["lanes"]["youtube"]["quality"].get(
                "captions", {}
            )
            previous_captions = (
                previous_results.get("lanes", {})
                .get("youtube", {})
                .get("quality", {})
                .get("captions", {})
            )
            ...
            if not current_captions or not previous_captions:
                return {"consistent": True, "reason": "No caption data to compare"}
            ...
            if current_method == "heuristic" and previous_method == "heuristic":
                ...
                if current_path and previous_path:
                    ...
                    if os.path.exists(current_srt_path) and os.path.exists(
                        previous_srt_path
                    ):
                        try:
            ...
            return {"consistent": True, "reason": "Caption consistency check passed"}
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Acceptance harness for content pipeline"
    )
    parser.add_argument(
        "--skip-orchestrator",
        action="store_true",
    ...
    parser.add_argument(
        "--output",
        default="acceptance_results.json",
