...
def test_texture_qa_integration():
    ...
    try:
        ...
        draw.rectangle([50, 50, 150, 150], fill='lightblue', outline='darkblue', width=3)
        ...
        print(f"Texture QA loop completed")
        ...
        if metadata['textures']['dialback_applied']:
            ...
            assert final_config['grain_strength'] < original_config['grain_strength'], "Grain strength should be reduced"
            assert final_config['posterize_levels'] > original_config['posterize_levels'], "Posterize levels should increase"
            assert final_config['halftone']['enable'] == False, "Halftone should be disabled on aggressive config"
