...
def test_micro_anim_limit():
    ...
    try:
        ...
        print(f"Configuration: max_elements_percent = {config['max_elements_percent']}%")
        ...
        for scene_data in test_scenes:
            ...
            animation_percent = (elements_animated / total_elements * 100) if total_elements > 0 else 0
            
            print(f"  Elements animated: {elements_animated}")
            print(f"  Animation percentage: {animation_percent:.1f}%")
            
            # Verify the 10% limit
            max_allowed = max(1, int(total_elements * config['max_elements_percent'] / 100))
            expected_max = min(max_allowed, total_elements)
            
            if elements_animated <= expected_max:
                print(f"  ✓ PASS: {elements_animated} ≤ {expected_max} (limit: {config['max_elements_percent']}%)")
            else:
                print(f"  ✗ FAIL: {elements_animated} > {expected_max} (limit: {config['max_elements_percent']}%)")
                all_passed = False
            ...
            if animation_percent <= config['max_elements_percent'] or elements_animated == 1:
                print(f"  ✓ PASS: {animation_percent:.1f}% (1 element minimum enforced)")
            else:
                print(f"  ✗ FAIL: {animation_percent:.1f}% > {config['max_elements_percent']}%")
                all_passed = False
