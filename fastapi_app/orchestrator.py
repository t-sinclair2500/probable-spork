...
class StageRunner:
    ...
    async def run_outline(self, job: Job, run_dir: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            from bin.core import load_brief, load_config
            ...
            if outline_path:
                ...
                target_path = self.storage.copy_pipeline_artifact(
                    str(outline_path), job.id, Stage.OUTLINE, "outline.json"
                )
                ...
                if target_path:
                    ...
                    self.log.info(
                        f"[outline] Completed outline generation for {job.slug}"
                    )
                    return {
                        "success": True,
    ...
    async def run_research(self, job: Job, run_dir: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            for artifact_path in research_artifacts:
                ...
                target_path = self.storage.copy_pipeline_artifact(
                    artifact_path, job.id, Stage.RESEARCH, filename
                )
    ...
    async def run_script(self, job: Job, run_dir: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if script_path:
                # Copy to job artifacts
                target_path = self.storage.copy_pipeline_artifact(
                    str(script_path), job.id, Stage.SCRIPT, "script.txt"
                )
                ...
                if target_path:
                    ...
                    self.log.info(
                        f"[script] Completed script generation for {job.slug}"
                    )
                    return {
                        "success": True,
    ...
    async def run_storyboard(self, job: Job, run_dir: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            run_storyboard(
                brief=brief_data, models_config=job.cfg.models, slug=job.slug
            )
            ...
            if storyboard_path:
                # Copy to job artifacts
                target_path = self.storage.copy_pipeline_artifact(
                    str(storyboard_path), job.id, Stage.STORYBOARD, "storyboard.json"
                )
                ...
                if target_path:
                    ...
                    self.log.info(
                        f"[storyboard] Completed storyboard planning for {job.slug}"
                    )
                    return {
                        "success": True,
            ...
            self.log.error(
                f"[storyboard] Failed storyboard planning for {job.slug}: {e}"
            )
            return {"success": False, "error": str(e)}
    ...
    async def run_assets(self, job: Job, run_dir: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if assets_dir:
                ...
                target_dir = self.storage.resolve_pipeline_paths(job.id, Stage.ASSETS)[
                    "stage_dir"
                ]
    ...
    async def run_animatics(self, job: Job, run_dir: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if animatics_dir:
                ...
                target_dir = self.storage.resolve_pipeline_paths(
                    job.id, Stage.ANIMATICS
                )["stage_dir"]
                ...
                self.log.info(
                    f"[animatics] Completed animatics generation for {job.slug}"
                )
                return {
                    "success": True,
            ...
            self.log.error(
                f"[animatics] Failed animatics generation for {job.slug}: {e}"
            )
            return {"success": False, "error": str(e)}
    ...
    async def run_audio(self, job: Job, run_dir: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            for audio_path in audio_files:
                ...
                target_path = self.storage.copy_pipeline_artifact(
                    audio_path, job.id, Stage.AUDIO, filename
                )
    ...
    async def run_assemble(self, job: Job, run_dir: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if video_path:
                # Copy to job artifacts
                target_path = self.storage.copy_pipeline_artifact(
                    str(video_path), job.id, Stage.ASSEMBLE, "video.mp4"
                )
        ...
        try:
            ...
            if acceptance_path:
                ...
                if target_path:
                    ...
                    self.log.info(
                        f"[acceptance] Completed acceptance validation for {job.slug}"
                    )
                    return {
                        "success": True,
            ...
            self.log.error(
                f"[acceptance] Failed acceptance validation for {job.slug}: {e}"
            )
            return {"success": False, "error": str(e)}
...
class Orchestrator:
    ...
    async def pause_for_gate(self, job_id: str, stage: Stage) -> None:
        ...
        with self.lock:
            ...
            if not gate:
                logger.error(
                    f"[orchestrator] Gate not found for stage {stage.value} in job {job_id}"
                )
                return
        ...
        try:
            ...
            for stage in stages[current_stage_index:]:
                ...
                if job.status == JobStatus.CANCELED:
                    logger.info(
                        f"[orchestrator] Job {job_id} cancelled at {stage.value}"
                    )
                    return
                ...
                logger.info(
                    f"[orchestrator] Executing {stage.value} stage for job {job_id}"
                )
                ...
                if not stage_result.get("success"):
                    ...
                    event_logger.stage_failed(
                        job.id, stage, stage_result.get("error", "Unknown error")
                    )

                    logger.error(
                        f"[orchestrator] Stage {stage.value} failed for job {job_id}"
                    )
                    return
                ...
                if self._stage_requires_gate(stage):
                    logger.info(
                        f"[orchestrator] Pausing job {job_id} at {stage.value} gate"
                    )
                ...
                logger.info(
                    f"[orchestrator] Completed {stage.value} stage for job {job_id}"
                )
    ...
    async def _execute_stage(
        self, stage: Stage, job: Job, run_dir: Path
    ) -> Dict[str, Any]:
    ...
    def _ensure_gate_exists(self, job: Job, stage: Stage):
        ...
        if not existing_gate:
            ...
            db.create_or_update_gate(job.id, stage, None, None, None, None, False)

            logger.info(
                f"[orchestrator] Created gate for {stage.value} stage in job {job.id}"
            )
    ...
    def _add_stage_artifacts(
        self, job: Job, stage: Stage, result: Dict[str, Any], run_dir: Path
    ):
    ...
    def _save_job_state(self, job: Job, run_dir: Path):
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "[orchestrator] Core module not available, skipping log_state call"
                )
    ...
    async def approve_gate(
        self, job_id: str, stage: Stage, operator: str, notes: Optional[str] = None
    ) -> bool:
    ...
    async def cleanup_completed_jobs(self):
        ...
        with self.lock:
            ...
            if completed_ids:
                logger.info(
                    f"[orchestrator] Cleaned up {len(completed_ids)} completed jobs"
                )
                    ...
                    if gate.approved is None and gate.required:
                        ...
                        if stage_config and stage_config.get("auto_approve"):
                            ...
                            if (
                                timeout_seconds
                                and gate.at
                                and (current_time - gate.at).total_seconds()
                                > timeout_seconds
                            ):
                                await self.auto_approve_gate(
                                    job_id,
                                    gate.stage,
                                    f"Auto-approved after {timeout_seconds} second timeout",
                                )
    ...
    async def auto_approve_gate(
        self, job_id: str, stage: Stage, reason: str = "Auto-approved by timeout"
    ):
        ...
        try:
            with self.lock:
                ...
                db.update_gate_decision(
                    job.id, stage, True, "timer", reason, None, True
                )
                ...
                all_gates_approved = all(
                    gate.approved for gate in job.gates if gate.required
                )
                ...
                if all_gates_approved:
                    ...
                    logger.info(
                        f"[orchestrator] Gate {stage.value} auto-approved for job {job_id}, resuming execution"
                    )
                    ...
                    logger.info(
                        f"[orchestrator] Gate {stage.value} auto-approved for job {job_id}"
                    )
            ...
            logger.error(
                f"[orchestrator] Failed to auto-approve gate {stage.value} for job {job_id}: {e}"
            )
            return False
    ...
    async def resume_job_execution(self, job_id: str):
        ...
        try:
            ...
            logger.error(
                f"[orchestrator] Failed to resume job execution for {job_id}: {e}"
            )
            return False
    ...
    async def resume_after_gate(self, job_id: str, stage: Stage) -> bool:
        ...
        try:
            with self.lock:
                if job_id not in self.active_jobs:
                    logger.warning(
                        f"[orchestrator] Job {job_id} not found in active jobs"
                    )
                    return False
                ...
                if not gate or gate.approved is not True:
                    logger.warning(
                        f"[orchestrator] Gate {stage.value} not approved for job {job_id}"
                    )
                    return False
                ...
                logger.info(
                    f"[orchestrator] Job {job_id} resumed after {stage.value} gate approval"
                )
                return True

        except Exception as e:
            logger.error(
                f"[orchestrator] Failed to resume job {job_id} after gate {stage.value}: {e}"
            )
            return False
