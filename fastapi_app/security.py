...
def validate_binding_config() -> bool:
    ...
    if not allow_external:
        if host not in ["127.0.0.1", "localhost", "::1"]:
            logger.error(f"[security] Security violation: External binding not allowed but host is {host}")
    ...
    if allow_external and host in ["0.0.0.0", "::"]:
        logger.warning("[security] External binding enabled - server will be accessible from any IP")
        logger.warning("[security] This is not recommended for production use")
    
    logger.info(f"[security] Server binding validated: {host} (external: {allow_external})")
    return True
...
def enforce_local_binding() -> str:
    ...
    if not allow_external:
        ...
        safe_host = "127.0.0.1"
        if host != safe_host:
            logger.warning(f"[security] Forcing local binding from {host} to {safe_host}")
...
def get_security_summary() -> Dict[str, Any]:
    ...
    return {
        ...
        "local_binding_enforced": not operator_config.get("server.allow_external_bind", False)
    }
...
def log_security_status():
    ...
    logger.info(f"[security]   Local binding enforced: {summary['local_binding_enforced']}")
    ...
    logger.info(f"[security]   Admin token: {'Set' if summary['admin_token_set'] else 'Using default'}")
    
    if not summary['local_binding_enforced']:
        logger.warning("[security] WARNING: External binding is enabled - server accessible from any IP")
    
    if not summary['admin_token_set']:
        logger.warning("[security] WARNING: Using default admin token - change this in production")
