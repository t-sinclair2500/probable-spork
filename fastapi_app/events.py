...
class EventStreamManager:
    ...
    async def broadcast_event(self, job_id: str, event: Event):
        ...
        if self.active_streams[job_id]:
            self.log.debug(
                f"[events] Broadcasted event {event.type} to {len(self.active_streams[job_id])} clients for job {job_id}"
            )
...
class EventLogger:
    ...
    async def emit_event(
        ...
        try:
            ...
            if db.add_event(job_id, event):
                ...
                self.log.error(
                    f"[events] Failed to add event to database: {event_type}"
                )
                return None
    ...
    def get_job_events(
        self, job_id: str, limit: int = 100, since: Optional[datetime] = None
    ) -> List[Event]:
    ...
    def gate_pause(
        self, job_id: str, stage: Stage, timeout_seconds: Optional[int] = None
    ):
    ...
    def gate_auto_approved(self, job_id: str, stage: Stage, reason: str):
        ...
        return self.emit_event_sync(
            ...
            payload={"stage": stage.value, "status": "auto_approved", "reason": reason},
        )
    ...
    def artifact_created(
        self, job_id: str, stage: Stage, artifact_type: str, path: str
    ):
