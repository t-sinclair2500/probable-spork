...
class StorageManager:
    ...
    def store_artifact(self, job_id: str, artifact: Artifact) -> bool:
        ...
        try:
            ...
            if os.path.exists(artifact.path):
                ...
                logger.info(
                    f"Stored artifact {artifact.kind} for {job_id} at {artifact_path}"
                )
                return True
            else:
    ...
    def get_artifact_path(self, job_id: str, stage: Stage, kind: str) -> Optional[Path]:
        ...
        try:
            ...
            logger.error(
                f"Failed to get artifact path for {job_id} - {stage} - {kind}: {e}"
            )
            return None
    ...
    def copy_pipeline_artifact(
        self, source_path: str, job_id: str, stage: Stage, filename: str
    ) -> Optional[Path]:
