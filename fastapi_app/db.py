...
from .models import Job, JobStatus, Stage, Gate, Artifact, Event
...
class Database:
    ...
    def get_job(self, job_id: str) -> Optional[Job]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                # Get gates
                gates_cursor = conn.execute("SELECT * FROM gates WHERE job_id = ?", (job_id,))
                gates = []
                for gate_row in gates_cursor.fetchall():
                    gate = Gate(
                        ...
                        at=datetime.fromisoformat(gate_row['decision_at']) if gate_row['decision_at'] else None,
                        notes=gate_row['notes'],
                        patch=json.loads(gate_row['patch_json']) if gate_row['patch_json'] else None,
                        auto_approved=bool(gate_row['auto_approved'])
                    )
                    gates.append(gate)
                
                # Get artifacts
                artifacts_cursor = conn.execute("SELECT * FROM artifacts WHERE job_id = ?", (job_id,))
                artifacts = []
    ...
    def update_job_status(self, job_id: str, status: JobStatus, stage: Optional[Stage] = None) -> bool:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                """, (status.value, stage.value, datetime.now(timezone.utc).isoformat(), job_id))
                else:
                    conn.execute("""
    ...
    def create_or_update_gate(self, job_id: str, stage: Stage, approved: bool, operator: str, notes: Optional[str] = None, patch: Optional[Dict[str, Any]] = None, auto_approved: bool = False) -> bool:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.commit()
                approval_type = "auto-approved" if auto_approved else ("approved" if approved else "rejected")
                logger.info(f"Gate {stage.value} for job {job_id} {approval_type} by {operator}")
                return True
        except Exception as e:
            logger.error(f"Failed to create/update gate decision for job {job_id} stage {stage}: {e}")
            return False

    def update_gate_decision(self, job_id: str, stage: Stage, approved: bool, operator: str, notes: Optional[str] = None, patch: Optional[Dict[str, Any]] = None, auto_approved: bool = False) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to update gate decision for job {job_id} stage {stage}: {e}")
            return False
    ...
    def store_gate_decision_file(self, job_id: str, stage: Stage, decision_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to store gate decision file for job {job_id} stage {stage}: {e}")
            return False

    def get_gate_decision_file(self, job_id: str, stage: Stage) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Failed to get gate decision file for job {job_id} stage {stage}: {e}")
            return None
