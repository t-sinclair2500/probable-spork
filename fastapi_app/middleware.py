...
class RateLimiter:
    ...
    def __init__(self):
        ...
        self.job_creations: Dict[str, Deque[float]] = defaultdict(lambda: deque(maxlen=100))
...
async def rate_limit_middleware(request: Request, call_next):
    ...
    if rate_limiting_enabled:
        ...
        api_limit = operator_config.get("security.rate_limiting.api_requests_per_minute", 60)
...
async def security_headers_middleware(request: Request, call_next):
    ...
    if security_headers_enabled:
        # HSTS header
        hsts_seconds = operator_config.get("security.security_headers.hsts_seconds", 31536000)
        response.headers["Strict-Transport-Security"] = f"max-age={hsts_seconds}; includeSubDomains"
        
        # Content Security Policy
        csp = operator_config.get("security.security_headers.content_security_policy", "default-src 'self'")
        response.headers["Content-Security-Policy"] = csp
        
        # X-Content-Type-Options
        xcto = operator_config.get("security.security_headers.x_content_type_options", "nosniff")
        ...
        # X-XSS-Protection
        xxp = operator_config.get("security.security_headers.x_xss_protection", "1; mode=block")
