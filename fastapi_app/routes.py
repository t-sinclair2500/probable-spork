...
async def get_operator_config(current_operator: str = Depends(get_current_operator)):
    ...
    config = {
        ...
        "security": {
            "rate_limiting": {
                ...
                "job_creation_per_minute": operator_config.get(
                    "security.rate_limiting.job_creation_per_minute"
                ),
                "api_requests_per_minute": operator_config.get(
                    "security.rate_limiting.api_requests_per_minute"
                ),
            },
            "cors": {
            ...
            "security_headers": {
                ...
                "hsts_seconds": operator_config.get(
                    "security.security_headers.hsts_seconds"
                ),
                "content_security_policy": operator_config.get(
                    "security.security_headers.content_security_policy"
                ),
                "x_content_type_options": operator_config.get(
                    "security.security_headers.x_content_type_options"
                ),
                "x_frame_options": operator_config.get(
                    "security.security_headers.x_frame_options"
                ),
                "x_xss_protection": operator_config.get(
...
async def validate_config(current_operator: str = Depends(get_current_operator)):
...
async def create_job(
    ...
    try:
        ...
        if job_create.brief:
            ...
            logger.info(
                f"[brief] Parsed free-text brief for job {job_id}: {free_text[:100]}..."
            )
            ...
            logger.info(
                f"[brief] Using default brief from conf/brief.yaml for job {job_id}"
            )
        ...
        try:
            ...
            cfg_snapshot = {
                "brief": brief_cfg,
                "render": (
                    global_cfg.dict() if hasattr(global_cfg, "dict") else global_cfg
                ),
                "models": (
                    global_cfg.dict() if hasattr(global_cfg, "dict") else global_cfg
                ),
            ...
            logger.warning(
                f"Failed to load pipeline configs: {e}, using minimal config"
            )
            cfg_snapshot = {
        ...
        if db.create_job(job):
            ...
            try:
                ...
                logger.info(
                    f"[brief] Persisted brief snapshot to {brief_snapshot_path}"
                )
            except Exception as e:
...
async def compile_brief(
    request: Dict[str, Any], current_operator: str = Depends(get_current_operator)
):
...
def _extract_assumptions(free_text: str, resolved_brief: Dict[str, Any]) -> List[str]:
    ...
    if "video" in resolved_brief:
        ...
        if (
            video_cfg.get("target_length_min") == 5
            and video_cfg.get("target_length_max") == 7
        ):
...
async def get_job(job_id: str, current_operator: str = Depends(get_current_operator)):
...
async def list_jobs(current_operator: str = Depends(get_current_operator)):
    """List all jobs"""
    try:
        jobs = db.list_jobs(limit=100)
        logger.info(
            f"Job listing requested by {current_operator}, returned {len(jobs)} jobs"
        )
        return jobs
    except Exception as e:
        ...
        for gate in job.gates:
            if gate.stage == stage:
                ...
                if db.update_gate_decision(
                    ...
                    event_logger.gate_approved(
                        job_id, stage, current_operator, gate_action.notes
                    )
                    ...
                    logger.info(
                        f"Gate {stage.value} approved for job {job_id} by {current_operator}"
                    )
        ...
        for gate in job.gates:
            if gate.stage == stage:
                ...
                if db.update_gate_decision(
                    ...
                    logger.info(
                        f"Gate {stage.value} rejected for job {job_id} by {current_operator}"
                    )
...
async def resume_job(
    job_id: str, current_operator: str = Depends(get_current_operator)
):
    ...
    try:
        ...
        if rejected_gates_with_patches:
            ...
            for gate in rejected_gates_with_patches:
                ...
                db.update_gate_decision(
                    job_id,
                    gate.stage,
                    True,
                    current_operator,
                    "Auto-approved after patch application",
                    None,
                    False,
                )
                ...
                decision_data = {
                    ...
                    "at": datetime.now(timezone.utc).isoformat(),
                    "notes": "Auto-approved after patch application",
                    "patch": gate.patch,
                    "auto_approved": False,
                }
        ...
        if await orchestrator.resume_after_gate(job_id, current_stage):
            logger.info(
                f"Job {job_id} resumed by {current_operator} with {len(rejected_gates_with_patches)} patches applied"
            )
            return {
                "message": "Job resumed successfully",
                "patches_applied": len(rejected_gates_with_patches),
            }
        else:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def apply_patch_to_artifact(job_id: str, stage: Stage, patch: Dict[str, Any]):
    ...
    try:
        ...
        if not stage_artifacts:
            logger.warning(
                f"No artifacts found for stage {stage.value} in job {job_id}"
            )
            return
        ...
        logger.error(
            f"Failed to apply patch to artifact for job {job_id} stage {stage}: {e}"
        )
        raise


async def apply_script_patch(
    job_id: str, artifacts: List[Artifact], patch: Dict[str, Any]
):
        ...
        if patch.get("type") == "text_replace":
            ...
            if section_marker and new_section:
                ...
                if section_marker in script_content:
                    ...
                    script_path.write_text(script_content, encoding="utf-8")
                    logger.info(
                        f"Applied section replacement patch to script {script_path}"
                    )
...
async def apply_storyboard_patch(
    job_id: str, artifacts: List[Artifact], patch: Dict[str, Any]
):
    ...
    try:
        ...
        storyboard_artifact = next(
            (a for a in artifacts if a.kind == "storyboard"), None
        )
                    ...
                    for section in storyboard_data.get("sections", []):
                        for beat in section.get("beats", []):
                            if beat.get("id") == beat_id:
                                ...
                                logger.info(
                                    f"Updated beat {beat_id} duration to {new_duration}"
                                )
                                break
...
async def get_job_events(
    ...
    try:
        ...
        return {
            ...
            "last_event_time": (
                event_responses[-1]["timestamp"] if event_responses else None
            ),
            "since": since,
            "timestamp": datetime.now(timezone.utc).isoformat(),
        }
    ...
    try:
        ...
        async def event_stream():
            ...
            try:
                ...
                logger.info(
                    f"[api] SSE stream started for job {job_id} by {current_operator}"
                )
                ...
                try:
                    ...
                    logger.warning(
                        f"[api] Cleanup error in SSE stream: {cleanup_error}"
                    )
...
async def get_gate_decision(
    job_id: str, stage: Stage, current_operator: str = Depends(get_current_operator)
):
...
async def apply_patch_direct(
    ...
    try:
        ...
        logger.info(
            f"Patch applied directly to {stage.value} for job {job_id} by {current_operator}"
        )
    ...
    try:
        ...
        for gate in job.gates:
            ...
            if gate.approved is None and gate.required:
                ...
                if stage_config:
                    status_info["timeout_seconds"] = stage_config.get(
                        "auto_approve_after_s"
                    )
                    status_info["auto_approve_enabled"] = stage_config.get(
                        "auto_approve", False
                    )
                    ...
                    if gate.at and status_info["timeout_seconds"]:
                        ...
                        status_info["time_remaining"] = max(
                            0, status_info["timeout_seconds"] - elapsed
                        )
...
async def get_patch_types(current_operator: str = Depends(get_current_operator)):
                ...
                "examples": [
                    {
                        ...
                        "example": {"type": "level_adjust", "level_change_db": 3},
                    }
                ],
            },
        },
...
async def health_check():
    ...
    try:
        ...
        return HealthResponse(
            ...
            timestamp=datetime.now(timezone.utc),
            version="0.1.0",
            services={"database": False, "storage": False, "configuration": False},
        )
