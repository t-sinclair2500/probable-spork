...
def test_basic_endpoints(config):
    ...
    try:
        ...
        response = requests.get(
            f"{config['api_base']}/config/operator", headers=headers, timeout=5
        )
    ...
    try:
        headers = {"Authorization": f"Bearer {config['admin_token']}"}
        response = requests.get(
            f"{config['api_base']}/jobs", headers=headers, timeout=5
        )
...
def test_authentication(config):
    ...
    try:
        ...
        if response.status_code == 401:
            ...
            log_warning(
                f"Authentication test inconclusive (got {response.status_code}, expected 401)"
            )
    except requests.exceptions.RequestException as e:
    ...
    try:
        ...
        if response.status_code == 401:
            ...
            log_warning(
                f"Invalid token test inconclusive (got {response.status_code}, expected 401)"
            )
    except requests.exceptions.RequestException as e:
