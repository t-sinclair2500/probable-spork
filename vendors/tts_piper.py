...
class PiperTTS:
    ...
    def __init__(self, config: Dict):
        ...
        # Rate mapping
        self.rate_map = {"slow": 0.8, "medium": 1.0, "fast": 1.2}
    ...
    def synthesize(
        self, text: str, output_path: Optional[Path] = None, **kwargs
    ) -> Path:
        """
    ...
    def _parse_ssml_lite(self, text: str) -> str:
        ...
        # Handle break tags
        text = re.sub(
            r'<break\s+time="(\d+)ms"\s*/>', lambda m: f" [PAUSE_{m.group(1)}ms] ", text
        )

        # Handle emphasis tags
        text = re.sub(
            r"<emphasis>(.*?)</emphasis>",
            lambda m: f" [EMPHASIS] {m.group(1)} [END_EMPHASIS] ",
            text,
        )

        # Handle prosody rate tags
        text = re.sub(
            r'<prosody\s+rate="(slow|medium|fast)">(.*?)</prosody>',
            lambda m: f" [RATE_{m.group(1).upper()}] {m.group(2)} [END_RATE] ",
            text,
        )
    ...
    def _normalize_loudness(self, audio_path: Path) -> Path:
        ...
        normalized_path = (
            audio_path.parent / f"{audio_path.stem}_normalized{audio_path.suffix}"
        )
