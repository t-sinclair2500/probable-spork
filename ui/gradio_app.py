...
class APIClient:
    ...
    def __init__(self, admin_token: str):
        ...
        self.headers = {**DEFAULT_HEADERS, "Authorization": f"Bearer {admin_token}"}
    ...
    def approve_gate(
        self, job_id: str, stage: str, notes: str = "", patch: Optional[Dict] = None
    ) -> Dict:
    ...
    def compile_brief(
        self, free_text: str, preset: str, testing_mode: str, seed: int
    ) -> Dict:
...
def create_config_launch_page(
    api_client: APIClient = None, event_streamer: EventStreamer = None
):
    ...
    with gr.Blocks(title="Config & Launch") as page:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                notes_input = gr.Textbox(
                    label="Notes", placeholder="Optional notes for this job", lines=3
                )

                create_btn = gr.Button("Create Job", variant="primary")
                job_result = gr.Textbox(
                    label="Job Creation Result", interactive=False, lines=2
                )
        ...
        # Wire up events
        validate_btn.click(
            fn=validate_config, outputs=[validation_result, validation_result]
        )
        ...
        gr.Markdown(
            "Enter a natural language brief and let the system parse it automatically"
        )
        ...
        with gr.Row():
            with gr.Column(scale=2):
                ...
                assumptions_display = gr.Markdown(
                    label="Assumptions Made", visible=False
                )
        ...
        def preview_brief(free_text, preset, testing_mode, seed):
            ...
            if not client:
                return (
                    "Please initialize API client first",
                    None,
                    None,
                    None,
                    gr.Row(visible=False),
        ...
        def run_quick_brief_job(
            free_text, preset, testing_mode, seed, compiled_brief_json
        ):
            ...
            try:
                ...
                result = client.create_job_with_brief(
                    slug,
                    "narrative_history",
                    compiled_brief,
                    preset,
        ...
        preview_btn.click(
            ...
            outputs=[
                brief_preview,
                assumptions_display,
                compiled_brief_state,
                run_btn,
                run_btn,
            ],
        ).then(fn=lambda: gr.Code(visible=True), outputs=[brief_preview]).then(
            fn=lambda: gr.Markdown(visible=True), outputs=[assumptions_display]
        ).then(
            fn=lambda: gr.Textbox(visible=True), outputs=[quick_brief_result]
        )
...
def create_job_console_page(
    ...
    def load_jobs():
        ...
        if not client:
            return gr.Dropdown(
                choices=["Please initialize API client first"], value=None
            )
    ...
    def build_gate_controls(job):
        ...
        for gate in gates:
            if (
                gate.get("stage") == current_stage
                and gate.get("approved") is None
                and gate.get("required", True)
            ):
                controls.append(
                    f"### Gate: {gate.get('stage', 'unknown').title()}\n**Required:** {gate.get('required', True)}\n**Status:** Waiting for decision"
                )
    ...
    def resume_job(job_id):
        ...
        try:
            ...
            if "error" not in result:
                return "Job resumed successfully!", "success"
            else:
    ...
    with gr.Blocks(title="Job Console") as page:
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                gate_patch_input = gr.Code(
                    label="JSON Patch (optional)", language="json", lines=5
                )
                ...
                gate_result = gr.Textbox(
                    label="Gate Action Result", interactive=False, lines=2
                )
            ...
            with gr.Column(scale=2):
                ...
                events_display = gr.Textbox(
                    label="Event Stream", lines=10, interactive=False
                )
        ...
        # Wire up events
        refresh_jobs_btn.click(fn=load_jobs, outputs=job_dropdown)
        ...
        resume_btn.click(
            fn=resume_job, inputs=[current_job_id], outputs=[gate_result, gate_result]
        )
...
def create_ui() -> gr.Blocks:
    ...
    with gr.Blocks(title="Probable Spork Operator Console") as interface:
        ...
        init_btn.click(
            fn=init_api_client, inputs=[admin_token_input], outputs=[auth_status]
        ).then(fn=on_auth_success, outputs=[main_interface])

        # Set default token
        interface.load(
            lambda: "default-admin-token-change-me", outputs=admin_token_input
        )
...
def launch_ui(port: int = 7860, share: bool = False, debug: bool = False):
    ...
    try:
        interface = create_ui()
        interface.launch(server_port=port, share=share, debug=debug, show_error=True)
    except Exception as e:
