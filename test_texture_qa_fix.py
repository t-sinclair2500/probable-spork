...
def test_texture_qa_fix():
    ...
    try:
        ...
        if metadata['textures']['dialback_applied']:
            print(f"  - Final grain strength: {metadata['textures']['final_params']['grain_strength']:.3f}")
            print(f"  - Final posterize levels: {metadata['textures']['final_params']['posterize_levels']}")
            print(f"  - Final halftone enabled: {metadata['textures']['final_params']['halftone']['enable']}")
        ...
        if metadata['textures']['applied'] and metadata['textures']['dialback_applied']:
            print("✓ Texture QA loop successfully applied dialback to fix contrast issues")
            return True
        else:
